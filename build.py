...
CALCULATOR_GROUPS = {
    ...
    "average-return-calculator": "Investment",
    "depreciation-calculator": "Investment",
...
def manual_calculators():
    return [{
        "slug": "average-return-calculator",
        "title": "Average Return Calculator",
        "cat": "Financial",
        "group": "Investment",
        "engine": "average_return_advanced",
        "desc": "Calculate a money-weighted average annual return from dated cash flows or annualize multiple holding-period returns.",
        "formula": "The cash-flow mode solves the annual discount rate that makes dated cash flows sum to zero; period mode compounds entered returns and annualizes them over total holding time.",
        "example": "Compare starting and ending account balances with deposits and withdrawals, or combine several investment return periods.",
        "inputs": [],
        "keyword_data": {},
        "seo_keyword": "average rate of return calculator",
        "source": "Google Search Console query opportunity",
    }]
...
def base_and_supplemental(data):
    ...
    manual = [c for c in manual_calculators() if c["slug"] not in existing_slugs]
    existing_slugs.update(c["slug"] for c in manual)
    ...
    calculators = existing + manual + cn_extra + extra
    ...
    for calc in calculators:
        ...
        if calc["slug"] == "depreciation-calculator":
            calc["cat"] = "Financial"
            calc["group"] = "Investment"
            calc["engine"] = "depreciation_advanced"
            calc["desc"] = "Calculate asset depreciation with straight-line, declining-balance, double-declining-balance, or sum-of-years-digits methods."
        if calc["slug"] == "conversion-calculator":
            calc["cat"] = "Conversion"
            calc["group"] = "General Unit Conversion"
            calc["engine"] = "unit_converter_advanced"
            calc["desc"] = "Convert common length, mass, temperature, volume, area, speed, pressure, energy, time, and digital data units."
        if calc["slug"] == "area-calculator":
            calc["cat"] = "Math"
            calc["group"] = "Geometry"
            calc["engine"] = "area_advanced"
            calc["desc"] = "Calculate area for rectangles, squares, triangles, circles, ellipses, parallelograms, trapezoids, and circular sectors."
...
def seo_title(calc):
    if calc.get("slug") == "area-calculator":
        return "Area Calculator: Rectangle, Triangle, Circle & More"
    if calc.get("slug") == "conversion-calculator":
        return "Conversion Calculator: Length, Weight, Volume & More"
    if calc.get("slug") == "depreciation-calculator":
        return "Depreciation Calculator: Straight-Line & Declining Balance"
    if calc.get("slug") == "triangle-calculator":
        return "Triangle Calculator: Sides, Angles, Area & Perimeter"
    if calc.get("slug") == "liters-to-cu-inches-calculator":
        return "Liters to Cubic Inches Calculator (L to cu in)"
    if calc.get("slug") == "weeks-to-months-calculator":
        return "Weeks to Months Calculator: Convert Weeks & Months"
    if calc.get("slug") == "sq-feet-to-acres-calculator":
        return "Square Feet to Acres Calculator (sq ft to Acres)"
...
def seo_description(calc):
    ...
    if calc.get("slug") == "area-calculator":
        return "Calculate area for rectangles, squares, triangles, circles, ellipses, parallelograms, trapezoids, and sectors. Convert square units and see formulas."
    if calc.get("slug") == "conversion-calculator":
        return "Convert length, weight, temperature, volume, area, speed, pressure, energy, time, and data units with formulas, reference factors, and reverse conversion."
    if calc.get("slug") == "depreciation-calculator":
        return "Calculate annual asset depreciation and book value using straight-line, declining balance, double declining balance, or sum-of-years-digits methods."
    if calc.get("slug") == "triangle-calculator":
        return "Solve a triangle from SSS, SAS, two right-triangle legs, or base and height. Find sides, angles, area, perimeter, altitudes, and radii."
    if calc.get("slug") == "liters-to-cu-inches-calculator":
        return "Convert liters to cubic inches or cubic inches to liters using exact definitions. Includes CID, cc, mL, cubic feet, gallons, formulas, and charts."
    if calc.get("slug") == "weeks-to-months-calculator":
        return "Convert weeks to months or months to weeks using average Gregorian, 30-day, or 4-week months. Compare days, years, formulas, examples, and charts."
    if calc.get("slug") == "sq-feet-to-acres-calculator":
        return "Convert square feet to acres or acres to square feet. Estimate rectangular lot acreage and compare square yards, meters, hectares, and square miles."
    if calc.get("slug") == "stones-to-pounds-calculator":
...
def triangle_input_html():
    return """<div class="triangle-fields">
<div class="field field-wide"><label for="triangle_mode">Known measurements</label><select id="triangle_mode"><option value="sss" selected>Three sides (SSS)</option><option value="sas">Two sides and included angle (SAS)</option><option value="right">Right triangle: two legs</option><option value="area">Base and perpendicular height</option></select></div>
<div class="fields triangle-entry" data-triangle-mode="sss"><div class="field"><label for="tri_sss_a">Side a</label><input id="tri_sss_a" type="number" step="any" min="0" value="3"></div><div class="field"><label for="tri_sss_b">Side b</label><input id="tri_sss_b" type="number" step="any" min="0" value="4"></div><div class="field field-wide"><label for="tri_sss_c">Side c</label><input id="tri_sss_c" type="number" step="any" min="0" value="5"></div></div>
<div class="fields triangle-entry is-hidden" data-triangle-mode="sas"><div class="field"><label for="tri_sas_a">Side a</label><input id="tri_sas_a" type="number" step="any" min="0" value="5"></div><div class="field"><label for="tri_sas_b">Side b</label><input id="tri_sas_b" type="number" step="any" min="0" value="7"></div><div class="field field-wide"><label for="tri_sas_angle">Included angle C</label><div class="input-unit"><input id="tri_sas_angle" type="number" step="any" min="0" max="180" value="60"><span>deg</span></div></div></div>
<div class="fields triangle-entry is-hidden" data-triangle-mode="right"><div class="field"><label for="tri_right_a">Leg a</label><input id="tri_right_a" type="number" step="any" min="0" value="3"></div><div class="field"><label for="tri_right_b">Leg b</label><input id="tri_right_b" type="number" step="any" min="0" value="4"></div></div>
<div class="fields triangle-entry is-hidden" data-triangle-mode="area"><div class="field"><label for="tri_area_base">Base</label><input id="tri_area_base" type="number" step="any" min="0" value="10"></div><div class="field"><label for="tri_area_height">Perpendicular height</label><input id="tri_area_height" type="number" step="any" min="0" value="6"></div></div>
<p class="field-note">Use the same linear unit for every length. Area is returned in the corresponding square unit.</p>
</div>"""


def depreciation_input_html():
    return """<div class="fields depreciation-fields">
<div class="field field-wide"><label for="dep_method">Depreciation method</label><select id="dep_method"><option value="straight" selected>Straight-line</option><option value="declining">Declining balance (custom rate)</option><option value="double">Double-declining balance</option><option value="syd">Sum-of-years-digits</option></select></div>
<div class="field"><label for="dep_cost">Asset cost</label><div class="input-unit"><input id="dep_cost" type="number" step="any" min="0" value="25000"><span>$</span></div></div>
<div class="field"><label for="dep_salvage">Estimated salvage value</label><div class="input-unit"><input id="dep_salvage" type="number" step="any" min="0" value="5000"><span>$</span></div></div>
<div class="field"><label for="dep_life">Useful life</label><div class="input-unit"><input id="dep_life" type="number" step="1" min="1" max="100" value="5"><span>years</span></div></div>
<div class="field"><label for="dep_year">Show result for year</label><input id="dep_year" type="number" step="1" min="1" max="100" value="3"></div>
<div class="field field-wide is-hidden" data-dep-rate><label for="dep_rate">Annual declining-balance rate</label><div class="input-unit"><input id="dep_rate" type="number" step="any" min="0" max="100" value="30"><span>%</span></div></div>
<p class="field-note field-wide">Uses full-year book-depreciation models. It does not apply MACRS classes, tax conventions, Section 179, bonus depreciation, or business-use limits.</p>
</div>"""


def unit_converter_input_html():
    return """<div class="fields unit-converter-fields">
<div class="field field-wide"><label for="unit_category">Measurement category</label><select id="unit_category"><option value="length" selected>Length</option><option value="mass">Mass / weight</option><option value="temperature">Temperature</option><option value="volume">Volume</option><option value="area">Area</option><option value="speed">Speed</option><option value="pressure">Pressure</option><option value="energy">Energy</option><option value="time">Time</option><option value="data">Digital data</option></select></div>
<div class="field"><label for="unit_value">Value</label><input id="unit_value" type="number" step="any" value="1"></div>
<div class="field"><label for="unit_from">From</label><select id="unit_from"><option value="in" selected>Inch (in)</option><option value="mm">Millimeter (mm)</option><option value="cm">Centimeter (cm)</option><option value="m">Meter (m)</option><option value="km">Kilometer (km)</option><option value="ft">Foot (ft)</option><option value="yd">Yard (yd)</option><option value="mi">Mile (mi)</option></select></div>
<div class="field"><label for="unit_to">To</label><select id="unit_to"><option value="mm" selected>Millimeter (mm)</option><option value="cm">Centimeter (cm)</option><option value="m">Meter (m)</option><option value="km">Kilometer (km)</option><option value="in">Inch (in)</option><option value="ft">Foot (ft)</option><option value="yd">Yard (yd)</option><option value="mi">Mile (mi)</option></select></div>
<div class="field unit-swap-field"><button class="btn secondary unit-swap-btn" type="button" data-unit-swap aria-label="Swap conversion units" title="Swap conversion units">&#8646; Swap units</button></div>
<p class="field-note field-wide">Choose a category, enter a value, then select the source and destination units. Volume options use US customary units where labeled.</p>
</div>"""


def area_calculator_input_html():
    return """<div class="fields geometry-area-fields">
<div class="field field-wide"><label for="geo_shape">Shape</label><select id="geo_shape"><option value="rectangle" selected>Rectangle</option><option value="square">Square</option><option value="triangle">Triangle: base and height</option><option value="sss">Triangle: three sides (Heron's formula)</option><option value="circle">Circle</option><option value="ellipse">Ellipse</option><option value="parallelogram">Parallelogram</option><option value="trapezoid">Trapezoid</option><option value="sector">Circular sector</option></select></div>
<div class="field"><label for="geo_input_unit">Input unit</label><select id="geo_input_unit"><option value="ft" selected>Feet (ft)</option><option value="in">Inches (in)</option><option value="yd">Yards (yd)</option><option value="mm">Millimeters (mm)</option><option value="cm">Centimeters (cm)</option><option value="m">Meters (m)</option><option value="km">Kilometers (km)</option></select></div>
<div class="field"><label for="geo_output_unit">Area result unit</label><select id="geo_output_unit"><option value="sqft" selected>Square feet (ft²)</option><option value="sqin">Square inches (in²)</option><option value="sqyd">Square yards (yd²)</option><option value="sqmm">Square millimeters (mm²)</option><option value="sqcm">Square centimeters (cm²)</option><option value="sqm">Square meters (m²)</option><option value="sqkm">Square kilometers (km²)</option><option value="acre">Acres</option><option value="ha">Hectares</option></select></div>
<div class="field" data-geo-shapes="rectangle"><label for="geo_length">Length</label><input id="geo_length" type="number" step="any" min="0" value="20"></div>
<div class="field" data-geo-shapes="rectangle"><label for="geo_width">Width</label><input id="geo_width" type="number" step="any" min="0" value="12"></div>
<div class="field is-hidden" data-geo-shapes="square"><label for="geo_side">Side length</label><input id="geo_side" type="number" step="any" min="0" value="10"></div>
<div class="field is-hidden" data-geo-shapes="triangle parallelogram"><label for="geo_base">Base</label><input id="geo_base" type="number" step="any" min="0" value="12"></div>
<div class="field is-hidden" data-geo-shapes="triangle parallelogram"><label for="geo_height">Perpendicular height</label><input id="geo_height" type="number" step="any" min="0" value="8"></div>
<div class="field is-hidden" data-geo-shapes="sss"><label for="geo_a">Side a</label><input id="geo_a" type="number" step="any" min="0" value="3"></div>
<div class="field is-hidden" data-geo-shapes="sss"><label for="geo_b">Side b</label><input id="geo_b" type="number" step="any" min="0" value="4"></div>
<div class="field is-hidden" data-geo-shapes="sss"><label for="geo_c">Side c</label><input id="geo_c" type="number" step="any" min="0" value="5"></div>
<div class="field is-hidden" data-geo-shapes="circle sector"><label for="geo_radius">Radius</label><input id="geo_radius" type="number" step="any" min="0" value="6"></div>
<div class="field is-hidden" data-geo-shapes="sector"><label for="geo_angle">Central angle</label><div class="input-unit"><input id="geo_angle" type="number" step="any" min="0" max="360" value="90"><span>deg</span></div></div>
<div class="field is-hidden" data-geo-shapes="ellipse"><label for="geo_semi_major">Semi-major axis</label><input id="geo_semi_major" type="number" step="any" min="0" value="8"></div>
<div class="field is-hidden" data-geo-shapes="ellipse"><label for="geo_semi_minor">Semi-minor axis</label><input id="geo_semi_minor" type="number" step="any" min="0" value="5"></div>
<div class="field is-hidden" data-geo-shapes="trapezoid"><label for="geo_base_one">Base a</label><input id="geo_base_one" type="number" step="any" min="0" value="10"></div>
<div class="field is-hidden" data-geo-shapes="trapezoid"><label for="geo_base_two">Base b</label><input id="geo_base_two" type="number" step="any" min="0" value="16"></div>
<div class="field is-hidden" data-geo-shapes="trapezoid"><label for="geo_trap_height">Perpendicular height</label><input id="geo_trap_height" type="number" step="any" min="0" value="7"></div>
<p class="field-note field-wide">All length inputs use the selected input unit. Height means the perpendicular distance to the base, not a slanted side.</p>
</div>"""


def liters_cubic_inches_input_html():
    return """<div class="liters-cubic-inches-fields">
<div class="field"><label for="liter_cuin_direction">Conversion direction</label><select id="liter_cuin_direction"><option value="liters_to_cuin" selected>Liters to cubic inches</option><option value="cuin_to_liters">Cubic inches to liters</option></select></div>
<div class="field"><label id="liter_cuin_value_label" for="liter_cuin_value">Volume in liters</label><div class="input-unit"><input id="liter_cuin_value" type="number" step="any" min="0" value="5"><span id="liter_cuin_unit">L</span></div></div>
<p class="field-note">1 cubic inch = exactly 0.016387064 liter. CID is another abbreviation for cubic-inch displacement.</p>
</div>"""


def weeks_months_input_html():
    return """<div class="weeks-months-fields">
<div class="field"><label for="weeks_months_direction">Conversion direction</label><select id="weeks_months_direction"><option value="weeks_to_months" selected>Weeks to months</option><option value="months_to_weeks">Months to weeks</option></select></div>
<div class="field"><label for="weeks_months_basis">Month definition</label><select id="weeks_months_basis"><option value="average" selected>Average Gregorian month</option><option value="thirty">30-day month</option><option value="four_weeks">4-week month</option></select></div>
<div class="field field-wide"><label id="weeks_months_value_label" for="weeks_months_value">Number of weeks</label><div class="input-unit"><input id="weeks_months_value" type="number" step="any" min="0" value="8"><span id="weeks_months_unit">weeks</span></div></div>
<p class="field-note">Calendar months contain 28 to 31 days. The average Gregorian month is 30.436875 days, or 4.348125 weeks.</p>
</div>"""


def square_feet_acres_input_html():
    return """<div class="square-feet-acres-fields">
<div class="field field-wide"><label for="acre_mode">Calculation type</label><select id="acre_mode"><option value="sqft_to_acres" selected>Square feet to acres</option><option value="acres_to_sqft">Acres to square feet</option><option value="dimensions">Lot length and width to acres</option></select></div>
<div class="fields acre-entry" data-acre-mode="sqft_to_acres"><div class="field field-wide"><label for="acre_sqft">Area in square feet</label><div class="input-unit"><input id="acre_sqft" type="number" step="any" min="0" value="43560"><span>sq ft</span></div></div></div>
<div class="fields acre-entry is-hidden" data-acre-mode="acres_to_sqft"><div class="field field-wide"><label for="acre_value">Area in acres</label><div class="input-unit"><input id="acre_value" type="number" step="any" min="0" value="1"><span>acres</span></div></div></div>
<div class="fields acre-entry is-hidden" data-acre-mode="dimensions"><div class="field"><label for="acre_length">Lot length</label><div class="input-unit"><input id="acre_length" type="number" step="any" min="0" value="200"><span>ft</span></div></div><div class="field"><label for="acre_width">Lot width</label><div class="input-unit"><input id="acre_width" type="number" step="any" min="0" value="100"><span>ft</span></div></div></div>
<p class="field-note">1 acre = exactly 43,560 square feet. Length x width applies to rectangular lots.</p>
</div>"""
...
def average_return_input_html():
    activities = [
        (1, "deposit", 1000, "2023-01-01"),
        (2, "deposit", 0, "2024-01-01"),
        (3, "withdrawal", 0, "2025-01-01"),
        (4, "deposit", 0, "2025-06-01"),
    ]
    activity_rows = "".join(f"""<fieldset class="return-row"><legend>Activity {index}</legend>
<div class="field"><label for="return_activity_{index}_type">Type</label><select id="return_activity_{index}_type"><option value="deposit"{' selected' if kind == 'deposit' else ''}>Deposit</option><option value="withdrawal"{' selected' if kind == 'withdrawal' else ''}>Withdrawal</option></select></div>
<div class="field"><label for="return_activity_{index}_amount">Amount</label><div class="input-unit"><input id="return_activity_{index}_amount" type="number" step="0.01" min="0" value="{amount}"><span>$</span></div></div>
<div class="field"><label for="return_activity_{index}_date">Date</label><input id="return_activity_{index}_date" type="date" value="{activity_date}"></div>
</fieldset>""" for index, kind, amount, activity_date in activities)
    periods = [(1, 8, 1), (2, -5, 1), (3, 12, 1), (4, 6, 1), (5, 0, 0)]
    period_rows = "".join(f"""<fieldset class="return-row period-return-row"><legend>Period {index}</legend>
<div class="field"><label for="return_period_{index}_rate">Return for period</label><div class="input-unit"><input id="return_period_{index}_rate" type="number" step="0.01" min="-99.99" value="{rate}"><span>%</span></div></div>
<div class="field"><label for="return_period_{index}_years">Holding time</label><div class="input-unit"><input id="return_period_{index}_years" type="number" step="0.01" min="0" value="{years}"><span>yr</span></div></div>
</fieldset>""" for index, rate, years in periods)
    return f"""<div class="average-return-fields"><div class="field field-wide"><label for="return_mode">Calculation method</label><select id="return_mode"><option value="cashflow" selected>Dated account cash flows</option><option value="periods">Multiple holding-period returns</option></select></div>
<div data-return-mode="cashflow"><div class="fields return-balance-fields"><div class="field"><label for="return_start_balance">Starting balance</label><div class="input-unit"><input id="return_start_balance" type="number" step="0.01" min="0.01" value="10000"><span>$</span></div></div><div class="field"><label for="return_start_date">Starting date</label><input id="return_start_date" type="date" value="2021-01-01"></div><div class="field"><label for="return_end_balance">Ending balance</label><div class="input-unit"><input id="return_end_balance" type="number" step="0.01" min="0" value="16000"><span>$</span></div></div><div class="field"><label for="return_end_date">Ending date</label><input id="return_end_date" type="date" value="2026-01-01"></div></div><div class="return-list">{activity_rows}</div></div>
<div data-return-mode="periods" class="is-hidden"><p class="field-note">Enter each total holding-period return and the length of that period. Rows with zero holding time are ignored.</p><div class="return-list">{period_rows}</div></div></div>"""
...
def high_value_calculator_copy(calc):
    if calc.get("slug") == "area-calculator":
        return """
<h2>Calculate the area of common 2D shapes</h2><p>Select a shape, enter its measurements in one linear unit, and choose the square unit for the answer. This calculator handles rectangles, squares, triangles from base and height, triangles from three sides, circles, ellipses, parallelograms, trapezoids, and circular sectors. It also reports useful supporting measurements such as perimeter, circumference, semiperimeter, or arc length when the entered dimensions determine them.</p>
<h2>What area measures</h2><p>Area is the amount of two-dimensional surface inside a boundary. Because two lengths are multiplied, area is expressed in square units such as square inches, square feet, square meters, acres, or hectares. A rectangle that is 20 feet long and 12 feet wide has 240 square feet of area. Its perimeter is a linear measurement of 64 feet; square feet and feet describe different quantities and cannot be interchanged.</p>
<h2>Rectangle and square area</h2><p>For a rectangle, multiply length by width. A square uses the same formula, but all four sides have the same length, so its area is the side length squared.</p><p class="formula">rectangle area = length x width<br>square area = side²</p><p>A 20-foot by 12-foot rectangle has area 20 x 12 = 240 square feet. Its perimeter is 2 x (20 + 12) = 64 feet. A square with a 10-foot side has area 100 square feet and perimeter 40 feet.</p>
<h2>Triangle area from base and height</h2><p>A triangle occupies half the area of a parallelogram with the same base and perpendicular height. The height must meet the base at a right angle. A slanted side is not the height unless it is perpendicular to the chosen base.</p><p class="formula">triangle area = base x perpendicular height / 2</p><p>A triangle with a 12-foot base and an 8-foot perpendicular height has area 48 square feet. Base and height alone do not determine the other two sides, so this mode cannot produce a unique perimeter.</p>
<h2>Triangle area from three sides</h2><p>When all three side lengths are known, choose the three-side option. The calculator first checks the triangle inequality: every pair of sides must add to more than the remaining side. It then uses Heron's formula, where s is the semiperimeter.</p><p class="formula">s = (a + b + c) / 2<br>triangle area = sqrt(s(s - a)(s - b)(s - c))</p><p>For sides 3, 4, and 5, the semiperimeter is 6 and the area is 6 square units. The perimeter is 12 units. Use the dedicated <a href="/triangle-calculator/">triangle calculator</a> when you also need angles, altitudes, inradius, or circumradius.</p>
<h2>Circle and circular sector area</h2><p>A circle's area equals pi times the radius squared. Circumference equals two times pi times the radius. A circular sector is a fraction of a complete circle, so its area and arc length are proportional to the central angle divided by 360 degrees.</p><p class="formula">circle area = pi x radius²<br>sector area = angle / 360 x pi x radius²<br>sector arc length = angle / 360 x 2 x pi x radius</p><p>A 90-degree sector is one quarter of a full circle. The sector perimeter shown in the results includes the curved arc plus two radii.</p>
<h2>Ellipse area</h2><p>An ellipse uses two radii: the semi-major axis from the center to the farthest edge and the semi-minor axis from the center to the nearest edge. Multiply the two semi-axis lengths by pi.</p><p class="formula">ellipse area = pi x semi-major axis x semi-minor axis</p><p>The ellipse perimeter has no simple elementary formula. This calculator uses Ramanujan's widely used approximation for the supporting perimeter result, while the area calculation itself is direct.</p>
<h2>Parallelogram and trapezoid area</h2><p>A parallelogram has the same area as a rectangle with the same base and perpendicular height. A trapezoid's area is its height multiplied by the average of the two parallel base lengths.</p><p class="formula">parallelogram area = base x perpendicular height<br>trapezoid area = (base a + base b) x height / 2</p><p>The calculator does not report a trapezoid perimeter because the two nonparallel side lengths are not provided. Likewise, base and height do not uniquely determine the slanted side lengths of a parallelogram.</p>
<h2>Convert square units correctly</h2><p>Linear conversion factors must be squared for area. One foot is exactly 0.3048 meter, so one square foot is 0.3048² = 0.09290304 square meter. Multiplying square feet by only 0.3048 would be incorrect. This calculator converts the computed area to square inches, square feet, square yards, square millimeters, square centimeters, square meters, square kilometers, acres, and hectares through square meters.</p><p>One acre is exactly 43,560 square feet under the international-foot relationship used here, and one hectare is 10,000 square meters. Use the <a href="/sq-feet-to-acres-calculator/">square feet to acres calculator</a> when you need lot dimensions and one-acre comparisons.</p>
<h2>Choosing measurements and rounding</h2><p>Use measurements taken in the same linear unit. If one dimension is in feet and another is in inches, convert one before calculating. The selected input unit applies to every length field on this page. The result is converted to the chosen area unit after the geometric formula is applied. Keep more precision during intermediate calculations and round only the final value to the accuracy supported by the original measurements.</p>
<h2>Formula and unit references</h2><p>The rectangle, square, triangle, circle, and trapezoid formulas match the <a href="https://openstax.org/books/prealgebra/pages/c-geometric-formulas" rel="external noopener">OpenStax geometric formulas reference</a>. The SI unit of area is the square meter, as described by the <a href="https://www.nist.gov/pml/owm/metric-si/si-units-area" rel="external noopener">National Institute of Standards and Technology</a>. NIST conversion tables provide the relationships used for US customary and metric area units. Construction estimates should still include waste, openings, material coverage, and local project requirements; use the <a href="/square-footage-calculator/">square footage calculator</a> for material overage and cost planning.</p>
<h2>Frequently asked questions</h2><h3>Is area the same as square footage?</h3><p>Square footage is area expressed specifically in square feet. Area can be expressed in any square unit.</p><h3>Can I calculate an irregular shape?</h3><p>Break it into rectangles, triangles, circles, or other supported pieces. Add non-overlapping areas and subtract cutouts. Confirm that every dimension uses the same scale.</p><h3>Why is triangle perimeter unavailable in base-height mode?</h3><p>Many different triangles can share the same base and height. The other side lengths are required for a unique perimeter.</p><h3>Does the calculator include material waste?</h3><p>No. It calculates geometric area. Flooring, tile, roofing, paint, and concrete projects require product-specific coverage and waste allowances.</p>"""
    if calc.get("slug") == "conversion-calculator":
        return """
<h2>Convert units in one calculator</h2><p>This unit conversion calculator handles ten common measurement categories without making you switch between separate tools. Select length, mass, temperature, volume, area, speed, pressure, energy, time, or digital data; enter a number; then choose the source and destination units. The result, reverse relationship, base-unit value, and a table of equivalent measurements update together.</p>
<h2>How unit conversion works</h2><p>Most conversions use a shared base unit. The calculator first converts the entered value to that base and then converts the base value to the requested unit. Length uses meters, mass uses kilograms, volume uses liters, area uses square meters, speed uses meters per second, pressure uses pascals, energy uses joules, time uses seconds, and digital data uses bytes.</p><p class="formula">result = entered value x source factor / destination factor</p><p>For example, one inch is exactly 0.0254 meter and one millimeter is 0.001 meter. Therefore, 1 inch x 0.0254 / 0.001 = 25.4 millimeters. Use the swap button to reverse the selected units while keeping the same entered value.</p>
<h2>Length conversions</h2><p>Length options include millimeters, centimeters, meters, kilometers, inches, feet, yards, and miles. The inch is defined as exactly 0.0254 meter, so a foot is exactly 0.3048 meter, a yard is exactly 0.9144 meter, and a mile is exactly 1,609.344 meters. These exact relationships support common searches such as <a href="/inches-to-millimeters-calculator/">inches to millimeters</a> and feet to meters.</p>
<h2>Mass and weight conversions</h2><p>The mass category includes milligrams, grams, kilograms, ounces, pounds, stones, US short tons, and metric tonnes. One international avoirdupois pound is exactly 0.45359237 kilogram, one ounce is one-sixteenth of a pound, and one stone is 14 pounds. In everyday US and UK language, people often say weight when converting pounds or kilograms, even though mass and force are technically different physical quantities. For stone notation with leftover pounds, use the dedicated <a href="/stones-to-pounds-calculator/">stones to pounds calculator</a>.</p>
<h2>Temperature conversion formulas</h2><p>Temperature is different from linear unit conversion because Celsius, Fahrenheit, and kelvin do not share the same zero point. The calculator converts the entered temperature to Celsius, then converts Celsius to the target scale. Values below absolute zero are rejected.</p><p class="formula">F = C x 9 / 5 + 32<br>C = (F - 32) x 5 / 9<br>K = C + 273.15</p><p>Water freezes at 0 degrees Celsius, 32 degrees Fahrenheit, or 273.15 kelvins under the familiar reference condition. A temperature difference is not converted in exactly the same way as an absolute temperature, so this page is intended for absolute scale readings.</p>
<h2>Volume conversions and the gallon warning</h2><p>Volume options include milliliters, liters, cubic meters, US teaspoons, tablespoons, fluid ounces, cups, pints, quarts, gallons, cubic inches, and cubic feet. The calculator labels the customary units as US units because an Imperial gallon is larger than a US gallon. One US liquid gallon is exactly 3.785411784 liters. One cubic inch is exactly 0.016387064 liter. For engine displacement or detailed equivalents, use the <a href="/liters-to-cu-inches-calculator/">liters to cubic inches calculator</a>.</p>
<h2>Area conversions</h2><p>Area factors are squared length factors. One square foot equals 0.09290304 square meter, but one foot equals 0.3048 meter; simply using 0.3048 for square feet would be incorrect. One acre is exactly 43,560 square feet, and one hectare is 10,000 square meters. The <a href="/sq-feet-to-acres-calculator/">square feet to acres calculator</a> can also estimate a rectangular lot from its dimensions.</p>
<h2>Speed, pressure, and energy</h2><p>Speed conversions include meters per second, kilometers per hour, miles per hour, feet per second, and knots. One mile per hour equals exactly 0.44704 meter per second. Pressure options include pascals, kilopascals, bar, pounds per square inch, standard atmospheres, and millimeters of mercury. Energy options include joules, kilojoules, thermochemical calories and kilocalories, watt-hours, kilowatt-hours, and British thermal units. Reference conditions and calorie or BTU definitions can vary in specialist work, so the labels identify the convention used by this calculator.</p>
<h2>Time and digital data</h2><p>Time options range from milliseconds through weeks and average Gregorian years. The average year used here is 365.2425 days; calendar years can contain 365 or 366 days. For calendar-style estimates, compare the assumptions in the <a href="/weeks-to-months-calculator/">weeks to months calculator</a>. Digital data includes bits and bytes plus decimal kilobytes, megabytes, gigabytes, and terabytes, as well as binary kibibytes, mebibytes, gibibytes, and tebibytes. Decimal units use powers of 1,000; binary IEC units use powers of 1,024.</p>
<h2>Accuracy, rounding, and authoritative factors</h2><p>The calculator retains more precision internally than it usually displays. A long decimal result does not mean the original measurement was equally precise. Report only the significant digits supported by your input and purpose. The conversion factors are based on established SI relationships and the <a href="https://www.nist.gov/pml/special-publication-811/nist-guide-si-appendix-b-conversion-factors" rel="external noopener">NIST Guide for the Use of the International System of Units</a>. NIST notes that conversion-factor precision and rounding should match the intended application. Verify regulated, scientific, medical, safety-critical, or commercial measurements against the governing standard.</p>
<h2>Frequently asked questions</h2><h3>Can I convert negative values?</h3><p>Yes for meaningful signed quantities such as temperature. Negative values may be mathematically converted in other categories, but they may not represent a physically valid length, mass, area, or volume.</p><h3>Why do US and metric results have many decimal places?</h3><p>Exact definitions often produce terminating or repeating decimal results. The calculator shows enough digits for comparison while preserving the full value for subsequent calculations.</p><h3>Are US fluid ounces the same as Imperial fluid ounces?</h3><p>No. This calculator's teaspoon, tablespoon, fluid ounce, cup, pint, quart, and gallon choices are explicitly US customary units.</p><h3>What is the difference between MB and MiB?</h3><p>One decimal megabyte is 1,000,000 bytes. One mebibyte is 1,048,576 bytes. Storage vendors commonly use decimal units, while operating systems and technical tools may display binary quantities.</p>"""
    if calc.get("slug") == "depreciation-calculator":
        return """
<h2>Calculate asset depreciation and book value</h2><p>Enter the asset's original cost, expected salvage value, useful life, and the year you want to review. Choose a depreciation method to calculate annual expense, accumulated depreciation, and ending book value. The full schedule lets you compare how quickly each method recognizes the same depreciable cost.</p>
<h2>Straight-line depreciation</h2><p>Straight-line depreciation allocates an equal amount in every full year. Subtract salvage value from cost to find the depreciable basis, then divide by useful life.</p><p class="formula">annual depreciation = (asset cost - salvage value) / useful life</p><p>For a $25,000 asset with a $5,000 salvage value and a five-year life, depreciable basis is $20,000. Straight-line expense is $4,000 each year. After year three, accumulated depreciation is $12,000 and ending book value is $13,000.</p>
<h2>Declining-balance depreciation</h2><p>Declining balance applies a fixed percentage to the asset's opening book value each year. Expense is larger when book value is high and becomes smaller over time. The calculator never reduces book value below the entered salvage value.</p><p class="formula">annual depreciation = opening book value x annual declining rate</p><p>A custom 30% rate on a $25,000 opening value produces $7,500 of first-year depreciation and a $17,500 ending book value. The second-year expense is 30% of $17,500, not 30% of the original cost.</p>
<h2>Double-declining-balance method</h2><p>Double-declining balance uses twice the straight-line rate and applies it to opening book value. For a five-year life, the straight-line rate is 20%, so the double-declining rate is 40%. Salvage value is not subtracted before calculating the first accelerated expense, but it acts as the book-value floor in this calculator.</p><p class="formula">double-declining rate = 2 / useful life<br>annual depreciation = opening book value x double-declining rate</p>
<h2>Sum-of-years-digits depreciation</h2><p>Sum-of-years-digits is another accelerated method. Add the useful-life digits to create a denominator, then multiply depreciable basis by a fraction whose numerator declines each year. For a five-year life, the denominator is 5 + 4 + 3 + 2 + 1 = 15.</p><p class="formula">year expense = (cost - salvage) x remaining-life fraction / sum of years</p><p>The year-one fraction is 5/15, year two is 4/15, and year five is 1/15. Unlike declining balance, the schedule allocates the entire depreciable basis to the entered salvage value by the end of useful life.</p>
<h2>Cost, salvage value, and useful life</h2><p><strong>Asset cost</strong> is the amount placed into the asset's book basis for this estimate. <strong>Salvage value</strong> is the expected residual value when useful life ends. <strong>Useful life</strong> is the number of periods over which the asset is allocated. These are accounting assumptions, not live market-value forecasts. Repairs, impairment, disposal proceeds, changes in estimates, and additional capitalized costs are outside this simplified model.</p>
<h2>Book depreciation is not automatically tax depreciation</h2><p>This calculator uses full-year mathematical schedules for planning and comparison. It does not calculate US federal tax depreciation. The <a href="https://www.irs.gov/publications/p946" rel="external noopener">IRS Publication 946</a> explains that MACRS results depend on the depreciation system, property class, placed-in-service date, recovery period, convention, method, business use, and possible elections. IRS percentage tables can also incorporate partial-year conventions and switches between methods. Use current tax guidance or a qualified tax professional for a return.</p>
<h2>Choosing a method</h2><p>Straight-line is useful when the asset's economic benefit is expected to be relatively even. Accelerated methods recognize more expense early and less later, which may better match assets that lose usefulness quickly. The <a href="https://openstax.org/books/principles-financial-accounting/pages/11-3-explain-and-apply-depreciation-methods-to-allocate-capitalized-costs" rel="external noopener">OpenStax financial accounting guide</a> compares straight-line and double-declining-balance calculations. The appropriate method depends on the accounting framework, asset pattern, policy, and reporting purpose.</p>
<h2>Frequently asked questions</h2><h3>What is depreciable basis?</h3><p>For straight-line and sum-of-years-digits calculations here, depreciable basis is cost minus salvage value.</p><h3>Can book value fall below salvage value?</h3><p>No. Every schedule caps depreciation so ending book value is not lower than the entered salvage value.</p><h3>Why does custom declining balance sometimes end above salvage value?</h3><p>A percentage applied to a shrinking balance may not fully depreciate the asset during the selected useful life. The schedule shows the remaining book value instead of forcing an unsupported final adjustment.</p><h3>Does depreciation equal market value loss?</h3><p>No. Depreciation allocates accounting cost. Market value can move differently because of condition, demand, obsolescence, inflation, and location. Use the <a href="/car-depreciation-calculator/">car depreciation calculator</a> for a vehicle-value projection rather than a book schedule.</p>"""
    if calc.get("slug") == "triangle-calculator":
        return """
<h2>How to solve a triangle</h2><p>A triangle is determined only when enough independent measurements are known. Choose <strong>three sides (SSS)</strong>, <strong>two sides and their included angle (SAS)</strong>, <strong>two perpendicular legs</strong> for a right triangle, or <strong>base and perpendicular height</strong> when you only need area. The calculator returns the measurements that can be uniquely derived from that choice instead of inventing missing information.</p>
<h2>Three-side (SSS) triangle calculator</h2><p>Enter side lengths a, b, and c. Each side must be positive, and any two sides must add to more than the third. This is the triangle inequality. For example, 1, 2, and 3 do not form a triangle because the two shorter lengths meet in a straight line rather than enclosing an area.</p><p>For a valid SSS triangle, the semiperimeter is half the perimeter. Heron's formula then finds area without requiring a height.</p><p class="formula">s = (a + b + c) / 2<br>area = sqrt(s(s - a)(s - b)(s - c))</p><p>The three angles are calculated with the law of cosines. Angle A is opposite side a, angle B is opposite side b, and angle C is opposite side c.</p>
<h2>Two sides and included angle (SAS)</h2><p>Use SAS when the known angle lies between the two known sides. The law of cosines finds the third side, and one-half times the two known sides times the sine of the included angle gives the area.</p><p class="formula">c = sqrt(a² + b² - 2ab cos(C))<br>area = ab sin(C) / 2</p><p>The angle must be greater than 0 degrees and less than 180 degrees. A zero-degree or 180-degree angle collapses the figure into a line.</p>
<h2>Right triangle calculator</h2><p>When the two legs meet at 90 degrees, the Pythagorean theorem finds the hypotenuse. The calculator also returns both acute angles, area, perimeter, altitudes, inradius, and circumradius.</p><p class="formula">c = sqrt(a² + b²)<br>area = ab / 2</p>
<h2>Base and height area calculator</h2><p>Enter a base and the perpendicular height measured from that base to the opposite vertex. This mode calculates area only. Base and height do not uniquely determine the other two sides, the three angles, or the perimeter because the top vertex can move parallel to the base while preserving the same height and area.</p><p class="formula">area = base x perpendicular height / 2</p><p>People sometimes search for the “surface area of a triangle,” but a flat triangle has an <strong>area</strong>. Surface area normally describes the total exposed area of a three-dimensional solid.</p>
<h2>Worked 3-4-5 triangle example</h2><p>For sides 3, 4, and 5, the semiperimeter is 6. Heron's formula gives sqrt(6 x 3 x 2 x 1) = <strong>6 square units</strong>. The perimeter is 12 units. The angles are approximately 36.8699 degrees, 53.1301 degrees, and 90 degrees, confirming that this is a right triangle.</p>
<h2>Altitudes, inradius, and circumradius</h2><p>Once all three sides and area K are known, each altitude follows from rearranging the base-height area formula: h-a = 2K/a, h-b = 2K/b, and h-c = 2K/c. The inradius is the radius of the circle tangent to all three sides, while the circumradius belongs to the circle passing through all three vertices.</p><p class="formula">inradius r = K / s<br>circumradius R = abc / (4K)</p>
<h2>Formula source and calculation method</h2><p>The calculator follows the law of cosines and Heron's formula summarized in the <a href="https://openstax.org/books/precalculus-2e/pages/8-key-equations" rel="external noopener">OpenStax Precalculus 2e key equations</a>. It keeps full JavaScript floating-point precision during intermediate steps and rounds only displayed results. Near-degenerate triangles can be sensitive to small input changes, so retain appropriate measurement precision.</p>
<h2>Frequently asked questions</h2><h3>Can three side lengths always make a triangle?</h3><p>No. All sides must be positive and the sum of any two sides must be greater than the remaining side.</p><h3>What units should I use?</h3><p>Use one consistent unit for every length. If sides are in feet, perimeter and radii are in feet and area is in square feet. The same logic applies to inches, meters, centimeters, or any other unit.</p><h3>Why does base and height mode not show perimeter?</h3><p>Many different triangles can share the same base and perpendicular height. More information is required to determine the sloping sides and perimeter.</p><h3>Which angle is C in SAS mode?</h3><p>Angle C is the included angle between sides a and b. The calculated side c lies opposite angle C.</p>"""
    if calc.get("slug") == "liters-to-cu-inches-calculator":
        liter_rows = "".join(
            f"<tr><td>{liters:g} L</td><td>{liters / 0.016387064:.6g} cu in</td><td>{liters * 1000:,.0f} cc</td></tr>"
            for liters in (0.5, 1, 1.5, 2, 2.5, 3, 3.5, 4, 5, 5.7, 6.2, 7, 8, 10)
        )
        cubic_rows = "".join(
            f"<tr><td>{cubic_inches:g} cu in</td><td>{cubic_inches * 0.016387064:.6g} L</td><td>{cubic_inches * 16.387064:,.3f} cc</td></tr>"
            for cubic_inches in (50, 100, 150, 200, 250, 300, 350, 400, 500)
        )
        return f"""
<h2>Convert liters and cubic inches in either direction</h2><p>Select <strong>Liters to cubic inches</strong> for metric volume or engine-displacement values stated in liters. Select <strong>Cubic inches to liters</strong> for US customary volume or cubic-inch displacement. The result also provides milliliters, cubic centimeters, cubic feet, US gallons, Imperial gallons, and cubic meters so the same physical volume can be compared without repeating conversions.</p>
<h2>Liters to cubic inches formula</h2><p>One cubic inch equals exactly 0.016387064 liter. Divide liters by that exact volume to obtain cubic inches.</p><p class="formula">cubic inches = liters / 0.016387064<br>cubic inches = liters x 61.0237440947</p><p>For example, 5 liters divided by 0.016387064 is approximately <strong>305.11872 cubic inches</strong>. Ten liters is approximately 610.23744 cubic inches.</p>
<h2>Cubic inches to liters formula</h2><p>Multiply cubic inches by 0.016387064 to reverse the conversion.</p><p class="formula">liters = cubic inches x 0.016387064</p><p>For example, 350 cubic inches is 350 x 0.016387064 = <strong>5.7354724 liters</strong>. The calculator uses the exact factor during arithmetic and rounds only the displayed result.</p>
<h2>Liters to cubic inches chart</h2><div class="table-scroll"><table class="data-table"><thead><tr><th>Liters</th><th>Cubic inches / CID</th><th>Cubic centimeters</th></tr></thead><tbody>{liter_rows}</tbody></table></div>
<h2>Cubic inches to liters chart</h2><div class="table-scroll"><table class="data-table"><thead><tr><th>Cubic inches</th><th>Liters</th><th>Cubic centimeters</th></tr></thead><tbody>{cubic_rows}</tbody></table></div>
<h2>What does CID mean?</h2><p>In automotive specifications, CID means cubic-inch displacement. It describes the combined swept volume of an engine's cylinders, not the exterior size of the engine or the amount of oil, coolant, or fuel it holds. A 5.0-liter engine is about 305.12 CID, while a 6.2-liter engine is about 378.35 CID. Manufacturers may use a rounded marketing label, so the advertised liter or CID name can differ slightly from displacement calculated from bore, stroke, and cylinder count.</p>
<h2>Liters, milliliters, and cubic centimeters</h2><p>One liter equals exactly 1,000 milliliters and 1,000 cubic centimeters. Therefore 1 mL and 1 cc represent the same volume. These metric relationships make it easy to check the result: multiply liters by 1,000 for both mL and cc. Cubic centimeters are common in small-engine and motorcycle displacement, while liters are common for larger engines and everyday container volumes.</p>
<h2>Why “liters to inches” is incomplete</h2><p>A liter measures three-dimensional volume, while an inch measures one-dimensional length. They cannot be converted directly. The compatible US unit is <strong>cubic inches</strong>, written cu in or in cubed. The same distinction applies to cubic feet and cubic meters. A container's length in inches cannot be determined from its volume alone because containers with the same capacity can have different shapes.</p>
<h2>Exact conversion basis</h2><p>The international inch is exactly 0.0254 meter. Cubing that length gives one cubic inch as exactly 0.000016387064 cubic meter, which equals 0.016387064 liter. The <a href="https://www.nist.gov/document/2026-hb-133-appendix-e" rel="external noopener">NIST Handbook 133 conversion tables</a> list the same relationship between cubic inches and liters. Because the definitions are exact, differences between calculators normally come from display rounding rather than the factor itself.</p>
<h2>US and Imperial gallons are different</h2><p>A US liquid gallon is exactly 3.785411784 liters, while an Imperial gallon is exactly 4.54609 liters. The result panel labels each separately. Do not use an unlabeled gallon conversion when the country or system is unknown; the Imperial gallon is about 20% larger than the US gallon.</p>
<h2>Volume versus liquid capacity</h2><p>Cubic inches describe geometric volume and apply to solids, enclosed spaces, engine displacement, or liquid capacity. Liters are accepted for volume in the metric system. A mathematical conversion does not account for unusable tank space, wall thickness, fill level, expansion, manufacturing tolerance, rounded product labels, or an engine's combustion-chamber details.</p>
<h2>Precision and rounding</h2><p>Choose decimal places appropriate to the source measurement. A container labeled 5 L may not justify a result shown to six decimal places, even though the conversion factor is exact. Keep extra precision during intermediate calculations and round the final value. For regulated packaging, laboratory work, engineering, or commerce, follow the required equipment and rounding rules.</p>
<h2>Frequently asked questions</h2><h3>How many cubic inches are in one liter?</h3><p>One liter is approximately 61.0237440947 cubic inches.</p><h3>How many cubic inches is a 5.0-liter engine?</h3><p>Five liters is approximately 305.11872 cubic inches, commonly rounded to 305 CID.</p><h3>How many cubic inches is a 6.2-liter engine?</h3><p>Six-point-two liters is approximately 378.34721 cubic inches.</p><h3>Are cc and mL the same volume?</h3><p>Yes. One cubic centimeter equals one milliliter, so one liter equals 1,000 cc and 1,000 mL.</p><h3>Can I convert liters directly to regular inches?</h3><p>No. Liters measure volume and inches measure length. You need cubic inches for a volume conversion or additional shape dimensions to find a length.</p>"""
    if calc.get("slug") == "weeks-to-months-calculator":
        weeks_rows = "".join(
            f"<tr><td>{weeks:g} week{'s' if weeks != 1 else ''}</td><td>{weeks * 7 / (365.2425 / 12):.6g} months</td><td>{weeks * 7:g} days</td></tr>"
            for weeks in (1, 2, 4, 6, 8, 10, 12, 13, 16, 20, 26, 40, 52)
        )
        months_rows = "".join(
            f"<tr><td>{months:g} month{'s' if months != 1 else ''}</td><td>{months * (365.2425 / 12) / 7:.6g} weeks</td><td>{months * (365.2425 / 12):.6g} days</td></tr>"
            for months in (1, 2, 3, 6, 9, 12, 18, 24)
        )
        return f"""
<h2>Convert weeks and months with a stated assumption</h2><p>A week always contains seven days, but a calendar month can contain 28, 29, 30, or 31 days. That means weeks and calendar months do not have one exact fixed conversion. This calculator makes the assumption visible: choose an average Gregorian month for general estimates, a 30-day month for a simplified planning convention, or a four-week month when that is the rule used by a program or schedule.</p>
<h2>Weeks to months formula</h2><p>The default method uses the average Gregorian year of 365.2425 days divided by 12. That makes an average month 30.436875 days, or 4.348125 weeks.</p><p class="formula">average months = weeks x 7 / 30.436875<br>average months = weeks / 4.348125</p><p>For example, 8 weeks contains 56 days. Dividing 56 by 30.436875 gives approximately <strong>1.83987 average months</strong>. This is a duration estimate, not a statement that every eight-week span crosses the same number of named calendar months.</p>
<h2>Months to weeks formula</h2><p>For the reverse calculation, multiply months by the selected number of days per month and divide by seven.</p><p class="formula">weeks = months x days per month / 7</p><p>Using the average Gregorian convention, one month is approximately <strong>4.348125 weeks</strong> and two months is approximately <strong>8.69625 weeks</strong>. Under a four-week convention, one month is exactly four weeks by definition, but that is shorter than most calendar months.</p>
<h2>Weeks to months chart</h2><p>The following table uses the average Gregorian month. Results are rounded for display.</p><div class="table-scroll"><table class="data-table"><thead><tr><th>Weeks</th><th>Average months</th><th>Days</th></tr></thead><tbody>{weeks_rows}</tbody></table></div>
<h2>Months to weeks chart</h2><div class="table-scroll"><table class="data-table"><thead><tr><th>Months</th><th>Average weeks</th><th>Average days</th></tr></thead><tbody>{months_rows}</tbody></table></div>
<h2>Average month versus calendar month</h2><p>The average-month method spreads the Gregorian calendar's 146,097 days in 400 years evenly across 4,800 months. It is useful for reporting, comparison, and approximate duration conversion. A real calendar interval must instead use actual start and end dates. January 15 to February 15 is one calendar month even though it contains 31 days; February 15 to March 15 can contain 28 or 29 days.</p>
<h2>Why 4 weeks is not usually one calendar month</h2><p>Four weeks equals exactly 28 days. Most calendar months are longer, so repeatedly treating a month as four weeks creates a growing difference. Twelve four-week periods total 336 days, which is about 29.24 days shorter than the average Gregorian year. A four-week month is appropriate only when a payroll cycle, benefits rule, subscription, pregnancy convention, or other system explicitly defines it that way.</p>
<h2>When a 30-day month is useful</h2><p>A 30-day convention makes one month equal to 4.285714 weeks. It can be useful for rough budgeting or systems whose written rules specify 30-day periods. Financial contracts may use specialized day-count conventions such as 30/360, but those rules affect interest accrual and should not be inferred from an ordinary weeks-to-months conversion.</p>
<h2>Calendar basis and source</h2><p>The default average follows the Gregorian calendar rather than the older Julian average of 365.25 days. The <a href="https://aa.usno.navy.mil/faq/leap_years" rel="external noopener">U.S. Naval Observatory explanation of leap years</a> states that the Gregorian pattern includes 97 leap days per 400 years, producing an average year of 365.2425 days. Dividing that average by 12 is a transparent convention for approximate month conversion.</p>
<h2>Pregnancy, age, and deadline calculations</h2><p>Pregnancy is commonly tracked in completed weeks because weeks are consistent while months vary. Converting pregnancy weeks to average months can be a helpful orientation, but clinical timing should remain in weeks and days and follow the provider's dating method. Age, legal deadlines, leases, medical schedules, and billing periods should also use actual dates when the rule says “calendar month.” Use the <a href="/date-calculator/">date calculator</a> or <a href="/time-duration-calculator/">time duration calculator</a> when start and end dates matter.</p>
<h2>Rounding guidance</h2><p>Keep several decimal places when one converted result feeds another calculation, then round the final answer for its purpose. Two decimal places are usually enough for a general estimate. Do not interpret extra displayed decimals as calendar certainty: the selected month definition, not arithmetic precision, is the largest source of difference.</p>
<h2>Frequently asked questions</h2><h3>How many months is 8 weeks?</h3><p>Eight weeks is about 1.83987 average Gregorian months, 1.86667 thirty-day months, or exactly two four-week months.</p><h3>How many weeks are in one month?</h3><p>An average Gregorian month is about 4.348125 weeks. An actual named calendar month spans four weeks plus zero to three days.</p><h3>Is 12 weeks three months?</h3><p>It is exactly three four-week periods, but about 2.7598 average Gregorian months. The calendar answer depends on the starting date.</p><h3>How many months is 52 weeks?</h3><p>Fifty-two weeks is 364 days, or about 11.9592 average Gregorian months.</p><h3>Which month definition should I choose?</h3><p>Use average Gregorian for general conversion, 30-day only when that convention is intended, and four-week only when the applicable schedule explicitly uses 28-day periods.</p>"""
    if calc.get("slug") == "sq-feet-to-acres-calculator":
        acre_rows = "".join(
            f"<tr><td>{acres:g} acre{'s' if acres != 1 else ''}</td><td>{acres * 43560:,.0f} sq ft</td><td>{acres / 640:.8g} sq mi</td></tr>"
            for acres in (0.1, 0.25, 0.5, 1, 2, 5, 10, 11, 20, 40)
        )
        sqft_rows = "".join(
            f"<tr><td>{sqft:,} sq ft</td><td>{sqft / 43560:.6g} acres</td><td>{sqft * 0.09290304:,.2f} sq m</td></tr>"
            for sqft in (1000, 2500, 5000, 10000, 20000, 43560, 50000, 100000, 435600, 1000000)
        )
        return f"""
<h2>Convert square feet and acres in either direction</h2><p>Use <strong>Square feet to acres</strong> when a deed, listing, floor plan, or site plan gives total area in square feet. Use <strong>Acres to square feet</strong> when you need the equivalent land area for a project estimate. The lot-dimensions mode first multiplies an entered rectangular length and width, then converts that square-foot total to acres.</p>
<h2>Square feet to acres formula</h2><p>One acre contains exactly 43,560 square feet. Divide square feet by 43,560 to convert an area to acres.</p><p class="formula">acres = square feet / 43,560</p><p>For example, 20,000 square feet divided by 43,560 is approximately <strong>0.459137 acres</strong>. The calculator keeps the full result internally and rounds only the displayed value.</p>
<h2>Acres to square feet formula</h2><p>Multiply acres by 43,560 to reverse the conversion.</p><p class="formula">square feet = acres x 43,560</p><p>For example, 2.5 acres is 2.5 x 43,560 = <strong>108,900 square feet</strong>. A quarter acre is 10,890 square feet, and a half acre is 21,780 square feet.</p>
<h2>Calculate acreage from lot dimensions</h2><p>For a rectangular parcel measured in feet, multiply length by width to find square feet, then divide by 43,560. A 200-foot by 100-foot lot contains 20,000 square feet, or about 0.459 acre. Real parcels may be irregular, curved, sloped, or described by bearings and distances; a simple rectangle is only appropriate when those entered dimensions represent the actual boundaries.</p><p class="formula">rectangular acres = length in feet x width in feet / 43,560</p>
<h2>Common acres to square feet chart</h2><div class="table-scroll"><table class="data-table"><thead><tr><th>Acres</th><th>Square feet</th><th>Square miles</th></tr></thead><tbody>{acre_rows}</tbody></table></div>
<h2>Common square feet to acres chart</h2><div class="table-scroll"><table class="data-table"><thead><tr><th>Square feet</th><th>Acres</th><th>Square meters</th></tr></thead><tbody>{sqft_rows}</tbody></table></div>
<h2>Equivalent land-area units</h2><p>The result panel also reports square yards, square meters, hectares, and square miles. One square yard contains 9 square feet. One square foot equals exactly 0.09290304 square meter under the international foot definition. A hectare contains 10,000 square meters, while one square mile contains 640 acres or 27,878,400 square feet.</p>
<h2>Why an acre is not a fixed shape</h2><p>An acre measures area, not length. It can be a square, rectangle, triangle, or irregular parcel as long as the enclosed area is 43,560 square feet. If one acre were a perfect square, each side would be about 208.71 feet, but that side length is not part of the definition. The current <a href="https://www.nist.gov/document/2026-hb-133-appendix-e" rel="external noopener">NIST Handbook 133 area tables</a> state the exact 43,560-square-foot relationship and explain that an acre need not be square or rectangular.</p>
<h2>Square miles versus miles</h2><p>Area must remain in squared units. Asking how many miles are in 11 acres mixes an area with a linear distance. The comparable area unit is square miles: 11 acres divided by 640 equals <strong>0.0171875 square mile</strong>. A boundary length in miles cannot be determined from acreage alone because parcels with the same area can have very different shapes and perimeters.</p>
<h2>Property records and survey accuracy</h2><p>Calculated acreage is only as reliable as the entered area or dimensions. Tax records, listing descriptions, recorded plats, geographic information systems, and physical surveys can differ because of rounding, easements, road rights-of-way, water boundaries, mapping methods, or whether gross and usable area are reported. For a purchase, boundary dispute, subdivision, permitting decision, or legal description, use the recorded documents and a licensed land surveyor rather than an online estimate.</p>
<h2>Related area calculations</h2><p>Use the <a href="/square-footage-calculator/">square footage calculator</a> when the area must first be found from room, wall, circle, or triangle dimensions. Use the <a href="/acres-to-square-feet-calculator/">acres to square feet calculator</a> for a dedicated reverse-conversion page. For material projects, do not treat acreage as usable coverage until setbacks, openings, slopes, waste, and site-specific exclusions have been considered.</p>
<h2>Frequently asked questions</h2><h3>How many square feet are in an acre?</h3><p>There are exactly 43,560 square feet in one acre.</p><h3>How many acres are in 10,000 square feet?</h3><p>10,000 divided by 43,560 is approximately 0.229568 acres.</p><h3>Is 200 feet by 200 feet one acre?</h3><p>No. That rectangle contains 40,000 square feet, which is about 0.918274 acre.</p><h3>How many acres are in a square mile?</h3><p>There are 640 acres in one square mile.</p><h3>Can I calculate an irregular lot with one length and width?</h3><p>Not accurately unless those dimensions truly describe an equivalent rectangle. Divide an irregular plan into measurable shapes or use survey/GIS area data.</p>"""
    if calc.get("slug") == "stones-to-pounds-calculator":
        chart_rows = "".join(
...
<h2>Money-weighted return from dated cash flows</h2><p>Cash-flow mode solves a money-weighted annual return, often called an annualized internal rate of return or XIRR. The starting balance and deposits are treated as money invested, while withdrawals and the ending balance are treated as money received. Each amount is discounted according to its exact number of days from the starting date. The solved rate makes the present value of all entered cash flows equal to zero.</p>
<p class="formula">0 = sum of cash flow / (1 + annual return)^(days from start / 365)</p>
<h2>Cash-flow example</h2><p>Suppose an account starts at $10,000 on January 1, 2021, receives a $1,000 deposit on January 1, 2023, and ends at $16,000 on January 1, 2026. Simply comparing $16,000 with $10,000 would count the added $1,000 as investment performance. The money-weighted calculation instead recognizes both the size and timing of that deposit before solving the annual rate.</p>
<h2>Multiple holding-period returns</h2><p>Period mode compounds the entered returns in sequence. A 10% gain followed by a 10% loss does not return to the starting value: 1.10 multiplied by 0.90 equals 0.99, a cumulative loss of 1%. The annualized result converts the combined growth factor into one equivalent yearly compound rate over the total holding time.</p>
<p class="formula">cumulative growth factor = product of (1 + each holding-period return)</p><p class="formula">annualized return = cumulative growth factor^(1 / total years) - 1</p>
<h2>Arithmetic average versus geometric return</h2><p>An arithmetic average adds the entered percentages and divides by the number of periods. It describes the average observation but does not reproduce the investment's compounded ending value when returns vary. The geometric annualized return incorporates compounding and is therefore better suited to comparing realized growth across different holding lengths. The results show both figures and label them separately.</p>
<h2>Total return, annualized return, and net gain</h2><p>Total or cumulative return measures the full gain or loss over the complete interval. Annualized return expresses that performance as an equivalent compound rate per year. Net gain is a dollar amount after adjusting the ending value for entered deposits and withdrawals. These measures answer different questions, so a high cumulative return over many years should not be compared directly with a one-year return.</p>
<h2>Why contribution timing matters</h2><p>A deposit made near the beginning has more time to experience investment performance than one made shortly before the ending date. Money-weighted return accounts for this difference. It also reflects the investor's timing decisions, which means it can differ from a fund's published time-weighted return even when both calculations are correct.</p>
<h2>Compounding and comparison</h2><p>Compound growth earns a return on both the original amount and prior gains. The <a href="https://www.investor.gov/introduction-investing" rel="external noopener">Investor.gov introduction to investing</a> explains this compounding effect. Annualizing can make holding periods easier to compare, but it does not remove differences in volatility, risk, taxes, fees, liquidity, or asset type.</p>
<h2>Accuracy and assumptions</h2><p>The cash-flow solver uses a 365-day year and searches for a real annual rate above -100%. Some unusual cash-flow patterns can have multiple mathematical solutions or no practical solution. This page reports one detected solution and warns when no bracketed solution is found. Enter account values after fees only if you want a return that reflects those fees, and use consistent treatment of dividends, taxes, and distributions.</p>
<h2>Average rate of return versus accounting ARR</h2><p>In capital budgeting, “average rate of return” can also mean accounting rate of return: average annual accounting profit divided by average investment. That metric uses accounting income and does not discount cash flows for timing. This calculator measures investment performance instead; it does not calculate project accounting ARR, net present value, or a business investment's depreciation-based book return.</p>
<h2>Frequently asked questions</h2><h3>Is annualized return the same as CAGR?</h3><p>For one beginning value, one ending value, and no intermediate cash flows, the annualized compound return is mathematically the same as CAGR. Dated deposits and withdrawals require a money-weighted calculation.</p><h3>Should deposits be entered as positive numbers?</h3><p>Yes. Select Deposit and enter a positive amount; the calculator applies the necessary cash-flow sign internally. Do the same for withdrawals.</p><h3>Can the annual return be negative?</h3><p>Yes. If the ending value and withdrawals do not compensate for invested amounts, the solved or compounded result can be below zero, but never below -100% for a standard real annual growth rate.</p><h3>Does this predict future returns?</h3><p>No. It measures the scenario entered. Historical or hypothetical performance does not guarantee a future result.</p>"""
    if calc.get("slug") == "debt-payoff-calculator":
        return """
...
def analysis_extra_html(calc):
    if calc.get("slug") == "area-calculator":
        return """<section class="mortgage-dashboard generic-dashboard geometry-area-dashboard" aria-label="Area calculation results">
<div class="section-head stack"><h2>Area Results</h2><p>Review the selected shape, area, supporting measurements, formula, and equivalent square units.</p></div>
<div class="summary-grid" id="genericSummary"></div>
<div class="chart-grid"><div class="chart-card compact-chart"><h3>Input Dimensions</h3><canvas id="genericChart" width="620" height="220" aria-label="Input dimension comparison" data-chart-type="bars"></canvas></div></div>
<div class="table-card"><h3>Calculation Details</h3><div class="table-scroll"><table class="data-table" id="genericTable"><thead><tr><th>Measurement</th><th>Value</th><th>Calculation note</th></tr></thead><tbody></tbody></table></div></div>
</section>"""
    if calc.get("slug") == "conversion-calculator":
        return """<section class="mortgage-dashboard generic-dashboard unit-converter-dashboard" aria-label="Unit conversion results">
<div class="section-head stack"><h2>Conversion Results</h2><p>Review the converted value, reverse relationship, base-unit value, and equivalents across the selected category.</p></div>
<div class="summary-grid" id="genericSummary"></div>
<div class="table-card"><h3>Equivalent Values</h3><div class="table-scroll"><table class="data-table" id="genericTable"><thead><tr><th>Unit</th><th>Equivalent value</th><th>Reference</th></tr></thead><tbody></tbody></table></div></div>
</section>"""
    if calc.get("slug") == "depreciation-calculator":
        return """<section class="mortgage-dashboard generic-dashboard depreciation-dashboard" aria-label="Asset depreciation results">
<div class="section-head stack"><h2>Depreciation Results</h2><p>Review annual expense, accumulated depreciation, book value, and the complete full-year schedule.</p></div>
<div class="summary-grid" id="genericSummary"></div>
<div class="chart-grid"><div class="chart-card compact-chart"><h3>Book Value by Year</h3><canvas id="genericChart" width="620" height="220" aria-label="Asset book value over useful life" data-chart-type="bars"></canvas></div></div>
<div class="table-card"><h3>Selected-Year Details</h3><div class="table-scroll"><table class="data-table" id="genericTable"><thead><tr><th>Measurement</th><th>Value</th><th>Calculation note</th></tr></thead><tbody></tbody></table></div></div>
<div class="table-card"><h3>Annual Depreciation Schedule</h3><div class="table-scroll"><table class="data-table" id="depreciationSchedule"><thead><tr><th>Year</th><th>Opening book value</th><th>Depreciation</th><th>Accumulated</th><th>Ending book value</th></tr></thead><tbody></tbody></table></div></div>
</section>"""
    if calc.get("slug") == "triangle-calculator":
        return """<section class="mortgage-dashboard generic-dashboard triangle-dashboard" aria-label="Triangle calculation results">
<div class="section-head stack"><h2>Triangle Results</h2><p>Review the solved sides, angles, area, perimeter, altitudes, and circle radii.</p></div>
<div class="summary-grid" id="genericSummary"></div>
<div class="chart-grid"><div class="chart-card compact-chart"><h3>Side Length Comparison</h3><canvas id="genericChart" width="620" height="220" aria-label="Triangle side length comparison" data-chart-type="bars"></canvas></div></div>
<div class="table-card"><h3>Calculation Details</h3><div class="table-scroll"><table class="data-table" id="genericTable"><thead><tr><th>Measurement</th><th>Value</th><th>Calculation note</th></tr></thead><tbody></tbody></table></div></div>
</section>"""
    if calc.get("slug") == "liters-to-cu-inches-calculator":
        return """<section class="mortgage-dashboard generic-dashboard liters-cubic-inches-dashboard" aria-label="Liters and cubic inches conversion results">
<div class="section-head stack"><h2>Volume Conversion Results</h2><p>Compare liters and cubic inches with metric, US customary, and Imperial volume units.</p></div>
<div class="summary-grid" id="genericSummary"></div>
<div class="chart-grid"><div class="chart-card compact-chart"><h3>Cubic-Inch Volume Comparison</h3><canvas id="genericChart" width="620" height="220" aria-label="Cubic-inch comparison with common liter volumes" data-chart-type="bars"></canvas></div></div>
<div class="table-card"><h3>Equivalent Volumes</h3><div class="table-scroll"><table class="data-table" id="genericTable"><thead><tr><th>Measurement</th><th>Value</th><th>Calculation note</th></tr></thead><tbody></tbody></table></div></div>
</section>"""
    if calc.get("slug") == "weeks-to-months-calculator":
        return """<section class="mortgage-dashboard generic-dashboard weeks-months-dashboard" aria-label="Weeks and months conversion results">
<div class="section-head stack"><h2>Time Conversion Results</h2><p>Compare the selected result with days, years, and alternate month-length conventions.</p></div>
<div class="summary-grid" id="genericSummary"></div>
<div class="chart-grid"><div class="chart-card compact-chart"><h3>Same Duration Under Different Month Definitions</h3><canvas id="genericChart" width="620" height="220" aria-label="Comparison of month conversion conventions" data-chart-type="bars"></canvas></div></div>
<div class="table-card"><h3>Conversion Details</h3><div class="table-scroll"><table class="data-table" id="genericTable"><thead><tr><th>Measurement</th><th>Value</th><th>Calculation note</th></tr></thead><tbody></tbody></table></div></div>
</section>"""
    if calc.get("slug") == "sq-feet-to-acres-calculator":
        return """<section class="mortgage-dashboard generic-dashboard square-feet-acres-dashboard" aria-label="Square feet and acres conversion results">
<div class="section-head stack"><h2>Area Conversion Results</h2><p>Compare acres and square feet with other common land-area units and review the exact calculation.</p></div>
<div class="summary-grid" id="genericSummary"></div>
<div class="chart-grid"><div class="chart-card compact-chart"><h3>Area Compared with One Acre</h3><canvas id="genericChart" width="620" height="220" aria-label="Entered area compared with common acre sizes" data-chart-type="bars"></canvas></div></div>
<div class="table-card"><h3>Equivalent Areas</h3><div class="table-scroll"><table class="data-table" id="genericTable"><thead><tr><th>Measurement</th><th>Value</th><th>Calculation note</th></tr></thead><tbody></tbody></table></div></div>
</section>"""
    if calc.get("slug") == "stones-to-pounds-calculator":
...
<div class="summary-grid" id="genericSummary"></div>
<div class="chart-grid"><div class="chart-card compact-chart"><h3 id="returnChartTitle">Account Values and Cash Flows</h3><canvas id="genericChart" width="620" height="220" aria-label="Average return calculation chart" data-chart-type="bars"></canvas></div></div>
<div class="table-card"><h3>Calculation Details</h3><div class="table-scroll"><table class="data-table" id="genericTable"><thead><tr><th>Metric</th><th>Value</th><th>Explanation</th></tr></thead><tbody></tbody></table></div></div>
</section>"""
    if calc.get("slug") == "debt-payoff-calculator":
...
def calculator_page(site, calc, related):
    if calc.get("slug") == "area-calculator":
        calc = {
            **calc,
            "desc": "Calculate area for nine common 2D shapes, convert square units, and review formulas and supporting measurements.",
            "formula": "Area depends on shape; for a rectangle, area = length x width.",
            "example": "A 20 ft by 12 ft rectangle has an area of 240 square feet.",
        }
    if calc.get("slug") == "conversion-calculator":
        calc = {
            **calc,
            "desc": "Convert common units across ten measurement categories with reverse conversion and an equivalent-values table.",
            "formula": "result = entered value x source-unit factor / destination-unit factor.",
            "example": "1 inch equals exactly 25.4 millimeters.",
        }
    if calc.get("slug") == "depreciation-calculator":
        calc = {
            **calc,
            "desc": "Calculate asset depreciation, accumulated depreciation, and book value with four common full-year accounting methods.",
            "formula": "Straight-line depreciation = (cost - salvage value) / useful life.",
            "example": "A $25,000 asset with $5,000 salvage value and a five-year life has $4,000 annual straight-line depreciation.",
        }
    if calc.get("slug") == "triangle-calculator":
        calc = {
            **calc,
            "desc": "Solve triangles from three sides, two sides and an included angle, two right-triangle legs, or base and height.",
            "formula": "Use Heron's formula, the law of cosines, or area = base x height / 2.",
            "example": "A 3-4-5 triangle has area 6, perimeter 12, and a 90-degree angle.",
        }
    if calc.get("slug") == "liters-to-cu-inches-calculator":
        calc = {
            **calc,
            "desc": "Convert liters to cubic inches or cubic inches to liters, including CID, cc, mL, cubic feet, and gallon equivalents.",
            "formula": "cubic inches = liters / 0.016387064.",
            "example": "5 liters is approximately 305.11872 cubic inches.",
        }
    if calc.get("slug") == "weeks-to-months-calculator":
        calc = {
            **calc,
            "desc": "Convert weeks to months or months to weeks using average Gregorian, 30-day, or four-week month conventions.",
            "formula": "average months = weeks x 7 / 30.436875.",
            "example": "8 weeks is approximately 1.83987 average Gregorian months.",
        }
    if calc.get("slug") == "sq-feet-to-acres-calculator":
        calc = {
            **calc,
            "desc": "Convert square feet to acres, reverse acres to square feet, or estimate rectangular lot acreage from length and width.",
            "formula": "acres = square feet / 43,560.",
            "example": "43,560 square feet equals exactly 1 acre.",
        }
    if calc.get("slug") == "stones-to-pounds-calculator":
        calc = {
            **calc,
    ...
    if calc.get("slug") == "area-calculator":
        fields = area_calculator_input_html()
        page_engine = "area_advanced"
    elif calc.get("slug") == "conversion-calculator":
        fields = unit_converter_input_html()
        page_engine = "unit_converter_advanced"
    elif calc.get("slug") == "depreciation-calculator":
        fields = depreciation_input_html()
        page_engine = "depreciation_advanced"
    elif calc.get("slug") == "triangle-calculator":
        fields = triangle_input_html()
        page_engine = "triangle_advanced"
    elif calc.get("slug") == "liters-to-cu-inches-calculator":
        fields = liters_cubic_inches_input_html()
        page_engine = "liters_cubic_inches_advanced"
    elif calc.get("slug") == "weeks-to-months-calculator":
        fields = weeks_months_input_html()
        page_engine = "weeks_months_advanced"
    elif calc.get("slug") == "sq-feet-to-acres-calculator":
        fields = square_feet_acres_input_html()
        page_engine = "square_feet_acres_advanced"
    elif calc.get("slug") == "stones-to-pounds-calculator":
        ...
        fields = average_return_input_html()
        page_engine = "average_return_advanced"
...
.unit-swap-field{align-self:end}.unit-swap-btn{width:100%;min-height:42px}.calculator-article:has(.unit-converter-fields) .calculator-layout.split-analysis{grid-template-columns:minmax(380px,440px) minmax(0,1fr)}.unit-converter-dashboard .table-card{margin-top:0}@media(max-width:900px){.calculator-article:has(.unit-converter-fields) .calculator-layout.split-analysis{grid-template-columns:minmax(0,1fr)}}
.geometry-area-fields .is-hidden{display:none!important}.calculator-article:has(.geometry-area-fields) .calculator-layout.split-analysis{grid-template-columns:minmax(390px,450px) minmax(0,1fr)}.geometry-area-dashboard .chart-grid{grid-template-columns:1fr}.geometry-area-dashboard .compact-chart canvas{max-height:220px}@media(max-width:900px){.calculator-article:has(.geometry-area-fields) .calculator-layout.split-analysis{grid-template-columns:minmax(0,1fr)}}
...
.average-return-fields{display:grid;gap:10px}.average-return-fields>.field-wide{grid-column:1/-1}.average-return-fields .is-hidden{display:none!important}.return-balance-fields{grid-template-columns:repeat(2,minmax(0,1fr));gap:8px!important}.return-list{display:grid;gap:7px;margin-top:8px}.return-row{display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:7px;margin:0;border:1px solid var(--line);border-radius:10px;padding:8px;background:#f8fafc}.return-row.period-return-row{grid-template-columns:repeat(2,minmax(0,1fr))}.return-row legend{padding:0 5px;color:var(--brand);font-size:12px;font-weight:850}.return-row .field label{font-size:12px!important;margin-bottom:3px}.return-row .field input,.return-row .field select,.return-row .input-unit input,.return-row .input-unit span{height:36px!important}.return-row .field input,.return-row .field select{padding:0 7px;font-size:13px!important}.return-row .input-unit span{padding:0 7px;font-size:12px}.average-return-dashboard .chart-grid{grid-template-columns:1fr}.average-return-dashboard .compact-chart canvas{max-height:230px}.calculator-article:has(.average-return-fields) .calculator-layout.split-analysis{grid-template-columns:minmax(460px,520px) minmax(0,1fr)}.calculator-article:has(.average-return-fields) .calc{padding:14px}.calculator-article:has(.average-return-fields) .calc h2{font-size:20px;margin-bottom:8px}.calculator-article:has(.average-return-fields) .calc-actions{margin-top:9px}.calculator-article:has(.average-return-fields) .result{padding:11px 13px;margin-top:8px;font-size:13px}.calculator-article:has(.average-return-fields) .result strong{font-size:25px}
@media(max-width:900px){.calculator-article:has(.average-return-fields) .calculator-layout.split-analysis{grid-template-columns:minmax(0,1fr)}}
@media(max-width:560px){.calculator-article:has(.average-return-fields) .calc{padding:10px}.return-balance-fields{gap:6px!important}.return-row{grid-template-columns:repeat(2,minmax(0,1fr));gap:5px;padding:6px}.return-row .field:last-child{grid-column:1/-1}.return-row.period-return-row .field:last-child{grid-column:auto}.return-row .field input,.return-row .field select,.return-row .input-unit input,.return-row .input-unit span{height:33px!important}.average-return-dashboard .summary-grid{grid-template-columns:repeat(2,minmax(0,1fr))}.calculator-article:has(.average-return-fields) .result strong{font-size:21px}}
...
.depreciation-fields .field-note{margin:0;padding:8px 9px;border-radius:7px;background:#f1f6fb;color:var(--muted);font-size:12px;line-height:1.4}.depreciation-dashboard .chart-grid{grid-template-columns:1fr}.depreciation-dashboard .compact-chart canvas{max-height:220px}.calculator-article:has(.depreciation-fields) .calculator-layout.split-analysis{grid-template-columns:minmax(360px,430px) minmax(0,1fr)}
@media(max-width:900px){.calculator-article:has(.depreciation-fields) .calculator-layout.split-analysis{grid-template-columns:minmax(0,1fr)}}
@media(max-width:560px){.calculator-article:has(.depreciation-fields) .calc{padding:10px}.depreciation-fields{gap:6px}.depreciation-fields .field label{font-size:12px!important}.depreciation-fields .field select,.depreciation-fields .field input{height:36px!important;padding:0 7px}.depreciation-dashboard .summary-grid{grid-template-columns:repeat(2,minmax(0,1fr))}.calculator-article:has(.depreciation-fields) .result strong{font-size:22px}}
.triangle-fields{display:grid;grid-template-columns:1fr;gap:9px}.triangle-entry{grid-column:1/-1}.triangle-fields>.field-note{margin:0;padding:8px 9px;border-radius:7px;background:#f1f6fb;color:var(--muted);font-size:12px;line-height:1.4}.triangle-dashboard .chart-grid{grid-template-columns:1fr}.triangle-dashboard .compact-chart canvas{max-height:220px}.calculator-article:has(.triangle-fields) .calculator-layout.split-analysis{grid-template-columns:minmax(350px,420px) minmax(0,1fr)}
@media(max-width:900px){.calculator-article:has(.triangle-fields) .calculator-layout.split-analysis{grid-template-columns:minmax(0,1fr)}}
@media(max-width:560px){.calculator-article:has(.triangle-fields) .calc{padding:10px}.triangle-fields,.triangle-entry{gap:6px}.triangle-fields .field label{font-size:12px!important}.triangle-fields .field select,.triangle-fields .field input{height:36px!important;padding:0 7px}.triangle-dashboard .summary-grid{grid-template-columns:repeat(2,minmax(0,1fr))}.calculator-article:has(.triangle-fields) .result strong{font-size:22px}}
.liters-cubic-inches-fields{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:9px}.liters-cubic-inches-fields>.field-note{grid-column:1/-1}.liters-cubic-inches-fields .field-note{margin:0;padding:8px 9px;border-radius:7px;background:#f1f6fb;color:var(--muted);font-size:12px;line-height:1.4}.liters-cubic-inches-dashboard .chart-grid{grid-template-columns:1fr}.liters-cubic-inches-dashboard .compact-chart canvas{max-height:220px}.calculator-article:has(.liters-cubic-inches-fields) .calculator-layout.split-analysis{grid-template-columns:minmax(360px,430px) minmax(0,1fr)}
@media(max-width:900px){.calculator-article:has(.liters-cubic-inches-fields) .calculator-layout.split-analysis{grid-template-columns:minmax(0,1fr)}}
@media(max-width:560px){.calculator-article:has(.liters-cubic-inches-fields) .calc{padding:10px}.liters-cubic-inches-fields{gap:6px}.liters-cubic-inches-fields .field label{min-height:28px;display:flex;align-items:end;font-size:12px!important}.liters-cubic-inches-fields .field select,.liters-cubic-inches-fields .field input{height:36px!important;padding:0 6px}.liters-cubic-inches-dashboard .summary-grid{grid-template-columns:repeat(2,minmax(0,1fr))}.calculator-article:has(.liters-cubic-inches-fields) .result strong{font-size:22px}}
.weeks-months-fields{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:9px}.weeks-months-fields>.field-wide,.weeks-months-fields>.field-note{grid-column:1/-1}.weeks-months-fields .field-note{margin:0;padding:8px 9px;border-radius:7px;background:#f1f6fb;color:var(--muted);font-size:12px;line-height:1.4}.weeks-months-dashboard .chart-grid{grid-template-columns:1fr}.weeks-months-dashboard .compact-chart canvas{max-height:220px}.calculator-article:has(.weeks-months-fields) .calculator-layout.split-analysis{grid-template-columns:minmax(360px,430px) minmax(0,1fr)}
@media(max-width:900px){.calculator-article:has(.weeks-months-fields) .calculator-layout.split-analysis{grid-template-columns:minmax(0,1fr)}}
@media(max-width:560px){.calculator-article:has(.weeks-months-fields) .calc{padding:10px}.weeks-months-fields{gap:6px}.weeks-months-fields .field label{min-height:28px;display:flex;align-items:end;font-size:12px!important}.weeks-months-fields .field select,.weeks-months-fields .field input{height:36px!important;padding:0 6px}.weeks-months-dashboard .summary-grid{grid-template-columns:repeat(2,minmax(0,1fr))}.calculator-article:has(.weeks-months-fields) .result strong{font-size:22px}}
.square-feet-acres-fields{display:grid;gap:9px}.square-feet-acres-fields>.field-wide{grid-column:1/-1}.acre-entry{grid-template-columns:repeat(2,minmax(0,1fr));gap:8px!important}.square-feet-acres-fields .is-hidden{display:none!important}.square-feet-acres-fields .field-note{margin:0;padding:8px 9px;border-radius:7px;background:#f1f6fb;color:var(--muted);font-size:12px;line-height:1.4}.square-feet-acres-dashboard .chart-grid{grid-template-columns:1fr}.square-feet-acres-dashboard .compact-chart canvas{max-height:220px}.calculator-article:has(.square-feet-acres-fields) .calculator-layout.split-analysis{grid-template-columns:minmax(350px,420px) minmax(0,1fr)}
@media(max-width:900px){.calculator-article:has(.square-feet-acres-fields) .calculator-layout.split-analysis{grid-template-columns:minmax(0,1fr)}}
@media(max-width:560px){.calculator-article:has(.square-feet-acres-fields) .calc{padding:10px}.acre-entry{gap:6px!important}.square-feet-acres-dashboard .summary-grid{grid-template-columns:repeat(2,minmax(0,1fr))}.calculator-article:has(.square-feet-acres-fields) .result strong{font-size:22px}}
...
function syncAverageReturnFields(){const mode=document.getElementById('return_mode')?.value||'cashflow';document.querySelectorAll('[data-return-mode]').forEach(el=>el.classList.toggle('is-hidden',el.dataset.returnMode!==mode));const title=document.getElementById('returnChartTitle');if(title)title.textContent=mode==='cashflow'?'Account Values and Cash Flows':'Compounded Value Index';return mode}
function returnDate(id){const raw=document.getElementById(id)?.value||'';return /^\d{4}-\d{2}-\d{2}$/.test(raw)?new Date(`${raw}T00:00:00`):null}
function datedReturnNpv(rate,flows,start){if(rate<=-1)return NaN;return flows.reduce((sum,item)=>sum+item.amount/Math.pow(1+rate,(item.date-start)/31536000000),0)}
function solveDatedReturn(flows,start){const points=[-.9999,-.99,-.9,-.75,-.5,-.25,-.1,-.05,0];for(let i=1;i<=240;i++)points.push(Math.pow(10,i/60)-1);let previous=points[0],fp=datedReturnNpv(previous,flows,start),roots=[];for(let i=1;i<points.length;i++){const current=points[i],fc=datedReturnNpv(current,flows,start);if(Number.isFinite(fc)&&Math.abs(fc)<1e-8)roots.push(current);if(Number.isFinite(fp)&&Number.isFinite(fc)&&fp*fc<0){let lo=previous,hi=current,flo=fp;for(let j=0;j<140;j++){const mid=(lo+hi)/2,fm=datedReturnNpv(mid,flows,start);if(Math.abs(fm)<1e-10){lo=hi=mid;break}if(flo*fm<=0)hi=mid;else{lo=mid;flo=fm}}roots.push((lo+hi)/2)}previous=current;fp=fc}return roots.length?roots.sort((a,b)=>Math.abs(a)-Math.abs(b))[0]:NaN}
function averageReturnProjection(){
  const mode=syncAverageReturnFields();
  if(mode==='periods'){const periods=[];for(let index=1;index<=5;index++){const years=V(`return_period_${index}_years`),rate=V(`return_period_${index}_rate`);if(years>0)periods.push({index,years,rate})}if(!periods.length)return{valid:false,mode,message:'Enter at least one holding period with a positive length.'};if(periods.some(item=>!Number.isFinite(item.rate)||item.rate<=-100))return{valid:false,mode,message:'Every active period return must be greater than -100%.'};const totalYears=periods.reduce((sum,item)=>sum+item.years,0),growth=periods.reduce((value,item)=>value*(1+item.rate/100),1),cumulative=(growth-1)*100,annualized=(Math.pow(growth,1/totalYears)-1)*100,arithmetic=periods.reduce((sum,item)=>sum+item.rate,0)/periods.length;let indexValue=100;const points=[{label:'Start',value:indexValue}];for(const item of periods){indexValue*=1+item.rate/100;points.push({label:`Period ${item.index}`,value:indexValue})}return{valid:true,mode,periods,totalYears,growth,cumulative,annualized,arithmetic,indexValue,points}}
  const start=returnDate('return_start_date'),end=returnDate('return_end_date'),starting=V('return_start_balance'),ending=V('return_end_balance');if(!start||!end||end<=start)return{valid:false,mode,message:'Choose an ending date after the starting date.'};if(!(starting>0)||ending<0)return{valid:false,mode,message:'Enter a positive starting balance and a nonnegative ending balance.'};const activities=[];for(let index=1;index<=4;index++){const amount=V(`return_activity_${index}_amount`),kind=document.getElementById(`return_activity_${index}_type`)?.value||'deposit',activityDate=returnDate(`return_activity_${index}_date`);if(amount>0){if(!activityDate||activityDate<start||activityDate>end)return{valid:false,mode,message:`Activity ${index} must have a date within the account period.`};activities.push({index,amount,kind,date:activityDate})}}
  const flows=[{label:'Starting balance',amount:-starting,date:start},...activities.map(item=>({label:`${item.kind==='deposit'?'Deposit':'Withdrawal'} ${item.index}`,amount:item.kind==='deposit'?-item.amount:item.amount,date:item.date})),{label:'Ending balance',amount:ending,date:end}],rate=solveDatedReturn(flows,start),years=(end-start)/31536000000,deposits=activities.filter(item=>item.kind==='deposit').reduce((sum,item)=>sum+item.amount,0),withdrawals=activities.filter(item=>item.kind==='withdrawal').reduce((sum,item)=>sum+item.amount,0),netInvested=starting+deposits-withdrawals,gain=ending-netInvested,simpleReturn=netInvested?gain/netInvested*100:0;if(!Number.isFinite(rate))return{valid:false,mode,message:'No bracketed real annual return was found for these dated cash flows.'};return{valid:true,mode,start,end,starting,ending,activities,flows,years,deposits,withdrawals,netInvested,gain,simpleReturn,annualized:rate*100};
}
...
const AREA_LENGTH_UNITS={mm:['Millimeter','mm',.001],cm:['Centimeter','cm',.01],m:['Meter','m',1],km:['Kilometer','km',1000],in:['Inch','in',.0254],ft:['Foot','ft',.3048],yd:['Yard','yd',.9144]};
const AREA_OUTPUT_UNITS={sqmm:['Square millimeter','mm²',1e-6],sqcm:['Square centimeter','cm²',1e-4],sqm:['Square meter','m²',1],sqkm:['Square kilometer','km²',1e6],sqin:['Square inch','in²',.00064516],sqft:['Square foot','ft²',.09290304],sqyd:['Square yard','yd²',.83612736],acre:['Acre','acre',4046.8564224],ha:['Hectare','ha',10000]};
function syncGeometryAreaFields(){const shape=document.getElementById('geo_shape')?.value||'rectangle';document.querySelectorAll('[data-geo-shapes]').forEach(el=>el.classList.toggle('is-hidden',!el.dataset.geoShapes.split(' ').includes(shape)));return shape}
function geometryAreaProjection(){const shape=syncGeometryAreaFields(),inputKey=document.getElementById('geo_input_unit')?.value||'ft',outputKey=document.getElementById('geo_output_unit')?.value||'sqft',inputUnit=AREA_LENGTH_UNITS[inputKey],outputUnit=AREA_OUTPUT_UNITS[outputKey];if(!inputUnit||!outputUnit)return{valid:false,message:'Choose valid input and output units.'};const positive=(id,label)=>{const value=V(id);if(!(value>0))throw new Error(`Enter a ${label.toLowerCase()} greater than zero.`);return value};let area=0,perimeter=null,formula='',shapeLabel='',measures=[];try{if(shape==='rectangle'){const l=positive('geo_length','Length'),w=positive('geo_width','Width');area=l*w;perimeter=2*(l+w);formula='A = length x width';shapeLabel='Rectangle';measures=[['Length',l],['Width',w]]}else if(shape==='square'){const s=positive('geo_side','Side length');area=s*s;perimeter=4*s;formula='A = side²';shapeLabel='Square';measures=[['Side',s],['Diagonal',s*Math.SQRT2]]}else if(shape==='triangle'){const b=positive('geo_base','Base'),h=positive('geo_height','Perpendicular height');area=b*h/2;formula='A = base x height / 2';shapeLabel='Triangle: base and height';measures=[['Base',b],['Height',h]]}else if(shape==='sss'){const a=positive('geo_a','Side a'),b=positive('geo_b','Side b'),c=positive('geo_c','Side c');if(a+b<=c||a+c<=b||b+c<=a)return{valid:false,message:'These sides fail the triangle inequality.'};const s=(a+b+c)/2;area=Math.sqrt(Math.max(0,s*(s-a)*(s-b)*(s-c)));perimeter=a+b+c;formula='A = sqrt(s(s - a)(s - b)(s - c))';shapeLabel='Triangle: three sides';measures=[['Side a',a],['Side b',b],['Side c',c]]}else if(shape==='circle'){const r=positive('geo_radius','Radius');area=Math.PI*r*r;perimeter=2*Math.PI*r;formula='A = pi x radius²';shapeLabel='Circle';measures=[['Radius',r],['Diameter',2*r]]}else if(shape==='ellipse'){const a=positive('geo_semi_major','Semi-major axis'),b=positive('geo_semi_minor','Semi-minor axis');area=Math.PI*a*b;perimeter=Math.PI*(3*(a+b)-Math.sqrt((3*a+b)*(a+3*b)));formula='A = pi x semi-major axis x semi-minor axis';shapeLabel='Ellipse';measures=[['Semi-major axis',a],['Semi-minor axis',b]]}else if(shape==='parallelogram'){const b=positive('geo_base','Base'),h=positive('geo_height','Perpendicular height');area=b*h;formula='A = base x height';shapeLabel='Parallelogram';measures=[['Base',b],['Height',h]]}else if(shape==='trapezoid'){const a=positive('geo_base_one','Base a'),b=positive('geo_base_two','Base b'),h=positive('geo_trap_height','Perpendicular height');area=(a+b)*h/2;formula='A = (base a + base b) x height / 2';shapeLabel='Trapezoid';measures=[['Base a',a],['Base b',b],['Height',h]]}else{const r=positive('geo_radius','Radius'),angle=V('geo_angle');if(!(angle>0&&angle<=360))return{valid:false,message:'Enter a central angle greater than 0 and no more than 360 degrees.'};const fraction=angle/360,arc=2*Math.PI*r*fraction;area=Math.PI*r*r*fraction;perimeter=arc+2*r;formula='A = angle / 360 x pi x radius²';shapeLabel='Circular sector';measures=[['Radius',r],['Arc length',arc]]}}catch(error){return{valid:false,message:error.message}}const squareMeters=area*inputUnit[2]*inputUnit[2],result=squareMeters/outputUnit[2],equivalents=Object.entries(AREA_OUTPUT_UNITS).map(([id,u])=>({id,name:u[0],symbol:u[1],value:squareMeters/u[2]}));return{valid:true,shape,shapeLabel,inputKey,outputKey,inputUnit,outputUnit,area,squareMeters,result,perimeter,formula,measures,equivalents}}
const UNIT_CATEGORIES={
length:{label:'Length',base:'m',defaults:['in','mm'],units:{mm:['Millimeter','mm',.001],cm:['Centimeter','cm',.01],m:['Meter','m',1],km:['Kilometer','km',1000],in:['Inch','in',.0254],ft:['Foot','ft',.3048],yd:['Yard','yd',.9144],mi:['Mile','mi',1609.344]}},
mass:{label:'Mass / weight',base:'kg',defaults:['oz','lb'],units:{mg:['Milligram','mg',1e-6],g:['Gram','g',.001],kg:['Kilogram','kg',1],oz:['Ounce','oz',.028349523125],lb:['Pound','lb',.45359237],st:['Stone','st',6.35029318],ton:['US short ton','US ton',907.18474],tonne:['Metric tonne','t',1000]}},
temperature:{label:'Temperature',base:'c',defaults:['f','c'],temperature:true,units:{c:['Celsius','°C',1],f:['Fahrenheit','°F',1],k:['Kelvin','K',1]}},
volume:{label:'Volume',base:'l',defaults:['gal','l'],units:{ml:['Milliliter','mL',.001],l:['Liter','L',1],m3:['Cubic meter','m³',1000],tsp:['US teaspoon','tsp',.00492892159375],tbsp:['US tablespoon','tbsp',.01478676478125],floz:['US fluid ounce','US fl oz',.0295735295625],cup:['US cup','cup',.2365882365],pt:['US pint','pt',.473176473],qt:['US quart','qt',.946352946],gal:['US gallon','US gal',3.785411784],cuin:['Cubic inch','cu in',.016387064],cuft:['Cubic foot','cu ft',28.316846592]}},
area:{label:'Area',base:'m2',defaults:['sqft','acre'],units:{sqmm:['Square millimeter','mm²',1e-6],sqcm:['Square centimeter','cm²',1e-4],m2:['Square meter','m²',1],ha:['Hectare','ha',10000],km2:['Square kilometer','km²',1e6],sqin:['Square inch','in²',.00064516],sqft:['Square foot','ft²',.09290304],sqyd:['Square yard','yd²',.83612736],acre:['Acre','acre',4046.8564224],sqmi:['Square mile','mi²',2589988.110336]}},
speed:{label:'Speed',base:'ms',defaults:['mph','kmh'],units:{ms:['Meter per second','m/s',1],kmh:['Kilometer per hour','km/h',1/3.6],mph:['Mile per hour','mph',.44704],fts:['Foot per second','ft/s',.3048],knot:['Knot','kn',.514444444444]}},
pressure:{label:'Pressure',base:'pa',defaults:['psi','kpa'],units:{pa:['Pascal','Pa',1],kpa:['Kilopascal','kPa',1000],bar:['Bar','bar',100000],psi:['Pound per square inch','psi',6894.757293168],atm:['Standard atmosphere','atm',101325],mmhg:['Millimeter of mercury','mmHg',133.322387415]}},
energy:{label:'Energy',base:'j',defaults:['kwh','j'],units:{j:['Joule','J',1],kj:['Kilojoule','kJ',1000],cal:['Thermochemical calorie','cal',4.184],kcal:['Kilocalorie','kcal',4184],wh:['Watt-hour','Wh',3600],kwh:['Kilowatt-hour','kWh',3.6e6],btu:['International Table BTU','BTU',1055.05585262]}},
time:{label:'Time',base:'s',defaults:['hour','min'],units:{ms:['Millisecond','ms',.001],s:['Second','s',1],min:['Minute','min',60],hour:['Hour','hr',3600],day:['Day','day',86400],week:['Week','wk',604800],year:['Average Gregorian year','yr',31556952]}},
data:{label:'Digital data',base:'byte',defaults:['gib','mb'],units:{bit:['Bit','bit',.125],byte:['Byte','B',1],kb:['Kilobyte','kB',1000],mb:['Megabyte','MB',1e6],gb:['Gigabyte','GB',1e9],tb:['Terabyte','TB',1e12],kib:['Kibibyte','KiB',1024],mib:['Mebibyte','MiB',1048576],gib:['Gibibyte','GiB',1073741824],tib:['Tebibyte','TiB',1099511627776]}}
};
function syncUnitConverterFields(force=false){const category=document.getElementById('unit_category'),from=document.getElementById('unit_from'),to=document.getElementById('unit_to');if(!category||!from||!to)return null;const key=category.value in UNIT_CATEGORIES?category.value:'length',config=UNIT_CATEGORIES[key];if(force||from.dataset.category!==key){const options=Object.entries(config.units).map(([id,u])=>`<option value="${id}">${u[0]} (${u[1]})</option>`).join('');from.innerHTML=options;to.innerHTML=options;from.value=config.defaults[0];to.value=config.defaults[1];from.dataset.category=key;to.dataset.category=key}return{key,config,from:from.value,to:to.value}}
function temperatureToCelsius(value,unit){return unit==='f'?(value-32)*5/9:unit==='k'?value-273.15:value}
function celsiusToTemperature(value,unit){return unit==='f'?value*9/5+32:unit==='k'?value+273.15:value}
function unitConversionProjection(){const selection=syncUnitConverterFields(),value=Number(document.getElementById('unit_value')?.value);if(!selection||!Number.isFinite(value))return{valid:false,message:'Enter a numeric value.'};const{key,config,from,to}=selection,fromUnit=config.units[from],toUnit=config.units[to];let base=0,result=0;if(config.temperature){base=temperatureToCelsius(value,from);if(base< -273.15-1e-10)return{valid:false,message:'Temperature cannot be below absolute zero.'};result=celsiusToTemperature(base,to)}else{base=value*fromUnit[2];result=base/toUnit[2]}const equivalents=Object.entries(config.units).map(([id,u])=>({id,name:u[0],symbol:u[1],value:config.temperature?celsiusToTemperature(base,id):base/u[2]}));const oneBase=config.temperature?temperatureToCelsius(1,from):fromUnit[2],oneResult=config.temperature?celsiusToTemperature(oneBase,to):oneBase/toUnit[2],reverseText=config.temperature?`0 ${fromUnit[1]} = ${F(celsiusToTemperature(temperatureToCelsius(0,from),to),10)} ${toUnit[1]}`:`1 ${toUnit[1]} = ${F(1/oneResult,10)} ${fromUnit[1]}`;return{valid:true,key,config,value,from,to,fromUnit,toUnit,base,result,oneResult,reverseText,equivalents}}
function swapUnitConverter(){const from=document.getElementById('unit_from'),to=document.getElementById('unit_to');if(!from||!to)return;const previous=from.value;from.value=to.value;to.value=previous;calc('unit_converter_advanced')}
function syncLitersCubicInchesFields(){const direction=document.getElementById('liter_cuin_direction')?.value||'liters_to_cuin',reverse=direction==='cuin_to_liters',label=document.getElementById('liter_cuin_value_label'),unit=document.getElementById('liter_cuin_unit');if(label)label.textContent=reverse?'Volume in cubic inches':'Volume in liters';if(unit)unit.textContent=reverse?'cu in':'L';return direction}
function litersCubicInchesProjection(){const direction=syncLitersCubicInchesFields(),value=Math.max(0,V('liter_cuin_value'));let liters=0,cubicInches=0;if(direction==='cuin_to_liters'){cubicInches=value;liters=cubicInches*0.016387064}else{liters=value;cubicInches=liters/0.016387064}const milliliters=liters*1000,cubicCentimeters=milliliters,cubicFeet=cubicInches/1728,usGallons=liters/3.785411784,imperialGallons=liters/4.54609,cubicMeters=liters/1000;return{direction,value,liters,cubicInches,milliliters,cubicCentimeters,cubicFeet,usGallons,imperialGallons,cubicMeters}}
function syncDepreciationFields(){const method=document.getElementById('dep_method')?.value||'straight';document.querySelectorAll('[data-dep-rate]').forEach(el=>el.classList.toggle('is-hidden',method!=='declining'));return method}
function depreciationProjection(){const method=syncDepreciationFields(),cost=V('dep_cost'),salvage=V('dep_salvage'),life=Math.round(V('dep_life')),requestedYear=Math.round(V('dep_year')),customRate=V('dep_rate')/100;if(!(cost>0))return{valid:false,message:'Enter an asset cost greater than zero.',method};if(!(salvage>=0&&salvage<cost))return{valid:false,message:'Salvage value must be zero or greater and less than asset cost.',method};if(!(life>=1&&life<=100))return{valid:false,message:'Useful life must be a whole number from 1 to 100 years.',method};if(method==='declining'&&!(customRate>0&&customRate<=1))return{valid:false,message:'Enter a declining-balance rate greater than 0% and no more than 100%.',method};const selectedYear=Math.max(1,Math.min(life,requestedYear||1)),basis=cost-salvage,sumYears=life*(life+1)/2,rate=method==='double'?2/life:method==='declining'?customRate:1/life,schedule=[];let book=cost,accumulated=0;for(let year=1;year<=life;year++){const opening=book,remaining=Math.max(0,opening-salvage);let expense=method==='straight'?basis/life:method==='syd'?basis*(life-year+1)/sumYears:opening*rate;expense=Math.max(0,Math.min(expense,remaining));accumulated+=expense;book=Math.max(salvage,opening-expense);schedule.push({year,opening,expense,accumulated,ending:book})}const selected=schedule[selectedYear-1],methodLabel=method==='straight'?'Straight-line':method==='declining'?'Declining balance':method==='double'?'Double-declining balance':'Sum-of-years-digits';return{valid:true,method,methodLabel,cost,salvage,life,selectedYear,basis,rate,schedule,selected,totalDepreciation:accumulated,endingBook:book}}
function syncTriangleFields(){const mode=document.getElementById('triangle_mode')?.value||'sss';document.querySelectorAll('[data-triangle-mode]').forEach(el=>el.classList.toggle('is-hidden',el.dataset.triangleMode!==mode));return mode}
function triangleProjection(){const mode=syncTriangleFields(),degrees=r=>r*180/Math.PI,clamp=x=>Math.max(-1,Math.min(1,x));if(mode==='area'){const base=V('tri_area_base'),height=V('tri_area_height');if(!(base>0&&height>0))return{valid:false,mode,message:'Enter a positive base and perpendicular height.'};return{valid:true,mode,base,height,area:base*height/2}}
let a=0,b=0,c=0,A=0,B=0,C=0;if(mode==='sss'){a=V('tri_sss_a');b=V('tri_sss_b');c=V('tri_sss_c');if(!(a>0&&b>0&&c>0))return{valid:false,mode,message:'Enter three positive side lengths.'};if(a+b<=c||a+c<=b||b+c<=a)return{valid:false,mode,message:'These lengths fail the triangle inequality: every two sides must add to more than the third.'};A=degrees(Math.acos(clamp((b*b+c*c-a*a)/(2*b*c))));B=degrees(Math.acos(clamp((a*a+c*c-b*b)/(2*a*c))));C=180-A-B}else if(mode==='sas'){a=V('tri_sas_a');b=V('tri_sas_b');C=V('tri_sas_angle');if(!(a>0&&b>0))return{valid:false,mode,message:'Enter two positive side lengths.'};if(!(C>0&&C<180))return{valid:false,mode,message:'The included angle must be greater than 0 degrees and less than 180 degrees.'};const rad=C*Math.PI/180;c=Math.sqrt(Math.max(0,a*a+b*b-2*a*b*Math.cos(rad)));A=degrees(Math.acos(clamp((b*b+c*c-a*a)/(2*b*c))));B=180-A-C}else{a=V('tri_right_a');b=V('tri_right_b');if(!(a>0&&b>0))return{valid:false,mode,message:'Enter two positive right-triangle leg lengths.'};c=Math.hypot(a,b);A=degrees(Math.atan2(a,b));B=90-A;C=90}
const perimeter=a+b+c,semiperimeter=perimeter/2,heron=Math.max(0,semiperimeter*(semiperimeter-a)*(semiperimeter-b)*(semiperimeter-c)),area=Math.sqrt(heron);if(!(area>0))return{valid:false,mode,message:'The measurements produce a degenerate triangle with no area.'};return{valid:true,mode,a,b,c,A,B,C,area,perimeter,semiperimeter,altitudeA:2*area/a,altitudeB:2*area/b,altitudeC:2*area/c,inradius:area/semiperimeter,circumradius:a*b*c/(4*area)}}
function syncWeeksMonthsFields(){const direction=document.getElementById('weeks_months_direction')?.value||'weeks_to_months',reverse=direction==='months_to_weeks',label=document.getElementById('weeks_months_value_label'),unit=document.getElementById('weeks_months_unit');if(label)label.textContent=reverse?'Number of months':'Number of weeks';if(unit)unit.textContent=reverse?'months':'weeks';return direction}
function weeksMonthsProjection(){const direction=syncWeeksMonthsFields(),basis=document.getElementById('weeks_months_basis')?.value||'average',value=Math.max(0,V('weeks_months_value')),daysPerMonth=basis==='thirty'?30:basis==='four_weeks'?28:365.2425/12;let weeks=0,months=0,days=0;if(direction==='months_to_weeks'){months=value;days=months*daysPerMonth;weeks=days/7}else{weeks=value;days=weeks*7;months=days/daysPerMonth}const years=days/365.2425,averageMonths=days/(365.2425/12),thirtyMonths=days/30,fourWeekMonths=days/28,basisLabel=basis==='thirty'?'30-day month':basis==='four_weeks'?'4-week month':'average Gregorian month';return{direction,basis,value,daysPerMonth,weeks,months,days,years,averageMonths,thirtyMonths,fourWeekMonths,basisLabel}}
function syncSquareFeetAcresFields(){const mode=document.getElementById('acre_mode')?.value||'sqft_to_acres';document.querySelectorAll('[data-acre-mode]').forEach(el=>el.classList.toggle('is-hidden',el.dataset.acreMode!==mode));return mode}
function squareFeetAcresProjection(){const mode=syncSquareFeetAcresFields();let squareFeet=0,length=0,width=0;if(mode==='acres_to_sqft')squareFeet=Math.max(0,V('acre_value'))*43560;else if(mode==='dimensions'){length=Math.max(0,V('acre_length'));width=Math.max(0,V('acre_width'));squareFeet=length*width}else squareFeet=Math.max(0,V('acre_sqft'));const acres=squareFeet/43560,squareYards=squareFeet/9,squareMeters=squareFeet*0.09290304,hectares=squareMeters/10000,squareMiles=squareFeet/27878400,sideFeet=Math.sqrt(squareFeet);return{mode,squareFeet,acres,squareYards,squareMeters,hectares,squareMiles,sideFeet,length,width}}
...
function calc(e){
 switch(e){
  ...
  case'area_advanced':{const p=geometryAreaProjection();if(!p.valid)show(`<strong>Unable to calculate area</strong><br>${p.message}`);else show(`<strong>${F(p.result,10)} ${p.outputUnit[1]}</strong><br>${p.shapeLabel}: ${F(p.area,10)} ${p.inputUnit[1]}²; ${p.perimeter===null?'perimeter needs more measurements':`${F(p.perimeter,8)} ${p.inputUnit[1]} perimeter`}.`);break}
  case'unit_converter_advanced':{const p=unitConversionProjection();if(!p.valid)show(`<strong>Unable to convert</strong><br>${p.message}`);else show(`<strong>${F(p.result,10)} ${p.toUnit[1]}</strong><br>${F(p.value,10)} ${p.fromUnit[1]} = ${F(p.result,10)} ${p.toUnit[1]}; 1 ${p.fromUnit[1]} = ${F(p.oneResult,10)} ${p.toUnit[1]}.`);break}
  case'depreciation_advanced':{const p=depreciationProjection();if(!p.valid)show(`<strong>Check the inputs</strong><br>${p.message}`);else show(`<strong>${USD(p.selected.expense)} depreciation in year ${p.selectedYear}</strong><br>${USD(p.selected.accumulated)} accumulated depreciation; ${USD(p.selected.ending)} ending book value using ${p.methodLabel.toLowerCase()}.`);break}
  case'triangle_advanced':{const p=triangleProjection();if(!p.valid)show(`<strong>Check the measurements</strong><br>${p.message}`);else if(p.mode==='area')show(`<strong>${F(p.area,8)} square units</strong><br>Area from base ${F(p.base,6)} and perpendicular height ${F(p.height,6)}; sides and perimeter need more information.`);else show(`<strong>${F(p.area,8)} square units</strong><br>Perimeter ${F(p.perimeter,8)}; sides ${F(p.a,6)}, ${F(p.b,6)}, ${F(p.c,6)}; angles ${F(p.A,4)}°, ${F(p.B,4)}°, ${F(p.C,4)}°.`);break}
  case'liters_cubic_inches_advanced':{const p=litersCubicInchesProjection(),main=p.direction==='cuin_to_liters'?`${F(p.liters,10)} liters`:`${F(p.cubicInches,10)} cubic inches`;show(`<strong>${main}</strong><br>${F(p.cubicCentimeters,6)} cc; ${F(p.cubicFeet,10)} cu ft; ${F(p.usGallons,8)} US gal; ${F(p.imperialGallons,8)} Imp gal.`);break}
  case'weeks_months_advanced':{const p=weeksMonthsProjection(),main=p.direction==='months_to_weeks'?`${F(p.weeks,8)} weeks`:`${F(p.months,8)} months`,article=p.basis==='average'?'an':'a';show(`<strong>${main}</strong><br>${F(p.days,6)} days; ${F(p.years,8)} average Gregorian years; using ${article} ${p.basisLabel}.`);break}
  case'square_feet_acres_advanced':{const p=squareFeetAcresProjection(),acreUnit=Math.abs(p.acres-1)<1e-12?'acre':'acres';show(`<strong>${F(p.acres,8)} ${acreUnit}</strong><br>${F(p.squareFeet,4)} sq ft; ${F(p.squareMeters,4)} sq m; ${F(p.hectares,8)} ha; ${F(p.squareMiles,10)} sq mi.`);break}
...
function renderGenericFromEngine(engine) {
  ...
  let cards=[], bars=[], rows=[];
  if (engine === "liters_cubic_inches_advanced") {
    const p=litersCubicInchesProjection(),source=p.direction==='cuin_to_liters'?`${F(p.value,10)} cu in`:`${F(p.value,10)} L`;
    cards=[["Cubic inches",`${F(p.cubicInches,10)} cu in`,"CID uses the same cubic-inch unit."],["Liters",`${F(p.liters,10)} L`,"Metric volume."],["Cubic centimeters",`${F(p.cubicCentimeters,6)} cc`,"Same numeric value as milliliters."],["US gallons",`${F(p.usGallons,8)} gal`,"US liquid gallons."]];
    bars=[{label:"Entered volume",value:p.cubicInches,display:`${F(p.cubicInches,4)} cu in`},{label:"1 liter",value:1/0.016387064,display:"61.0237 cu in"},{label:"5 liters",value:5/0.016387064,display:"305.119 cu in"},{label:"10 liters",value:10/0.016387064,display:"610.237 cu in"}];
    rows=[["Entered volume",source,"Selected conversion direction."],["Exact relationship","1 cu in = 0.016387064 L","International inch and liter definitions."],["Liters",`${F(p.liters,12)} L`,"Cubic inches x 0.016387064."],["Cubic inches / CID",`${F(p.cubicInches,12)} cu in`,"Liters divided by 0.016387064."],["Milliliters",`${F(p.milliliters,8)} mL`,"Liters x 1,000."],["Cubic centimeters",`${F(p.cubicCentimeters,8)} cc`,"Equal to milliliters."],["Cubic feet",`${F(p.cubicFeet,12)} cu ft`,"Cubic inches divided by 1,728."],["US liquid gallons",`${F(p.usGallons,10)} gal`,"Liters divided by 3.785411784."],["Imperial gallons",`${F(p.imperialGallons,10)} Imp gal`,"Liters divided by 4.54609."],["Cubic meters",`${F(p.cubicMeters,12)} cu m`,"Liters divided by 1,000."]];
  } else if (engine === "weeks_months_advanced") {
    const p=weeksMonthsProjection(),sourceUnit=p.direction==='months_to_weeks'?'months':'weeks',targetUnit=p.direction==='months_to_weeks'?'weeks':'months';
    cards=[[targetUnit.charAt(0).toUpperCase()+targetUnit.slice(1),`${F(p.direction==='months_to_weeks'?p.weeks:p.months,8)} ${targetUnit}`,`Using ${p.basis==='average'?'an':'a'} ${p.basisLabel}.`],["Days",`${F(p.days,6)} days`,"Duration normalized to seven-day weeks."],["Weeks",`${F(p.weeks,8)} weeks`,"Days divided by seven."],["Years",`${F(p.years,8)} years`,"Days divided by 365.2425."]];
    bars=[{label:"Average Gregorian",value:p.averageMonths,display:`${F(p.averageMonths,6)} months`},{label:"30-day months",value:p.thirtyMonths,display:`${F(p.thirtyMonths,6)} months`},{label:"4-week months",value:p.fourWeekMonths,display:`${F(p.fourWeekMonths,6)} months`}];
    rows=[["Entered duration",`${F(p.value,8)} ${sourceUnit}`,"Selected conversion direction."],["Selected month basis",p.basisLabel,`${F(p.daysPerMonth,6)} days per month.`],["Converted result",`${F(p.direction==='months_to_weeks'?p.weeks:p.months,10)} ${targetUnit}`,"Uses the selected month definition."],["Days",`${F(p.days,8)} days`,"Common duration used for conversion."],["Weeks",`${F(p.weeks,10)} weeks`,"Days divided by 7."],["Average Gregorian months",`${F(p.averageMonths,10)} months`,"Days divided by 30.436875."],["30-day months",`${F(p.thirtyMonths,10)} months`,"Days divided by 30."],["4-week months",`${F(p.fourWeekMonths,10)} months`,"Days divided by 28."],["Average Gregorian years",`${F(p.years,10)} years`,"Days divided by 365.2425."]];
  } else if (engine === "square_feet_acres_advanced") {
    const p=squareFeetAcresProjection(),source=p.mode==='dimensions'?`${F(p.length,4)} ft x ${F(p.width,4)} ft`:p.mode==='acres_to_sqft'?`${F(V('acre_value'),8)} acres`:`${F(V('acre_sqft'),4)} sq ft`;
    cards=[["Acres",`${F(p.acres,8)} ${Math.abs(p.acres-1)<1e-12?'acre':'acres'}`,"Square feet divided by 43,560."],["Square feet",`${F(p.squareFeet,4)} sq ft`,p.mode==='dimensions'?"Length multiplied by width.":"Equivalent land area."],["Square meters",`${F(p.squareMeters,4)} sq m`,"Square feet x 0.09290304."],["Square miles",`${F(p.squareMiles,10)} sq mi`,"Acres divided by 640."]];
    bars=[{label:"Entered area",value:p.acres,display:`${F(p.acres,6)} acres`},{label:"Quarter acre",value:.25,display:"0.25 acre"},{label:"Half acre",value:.5,display:"0.5 acre"},{label:"One acre",value:1,display:"1 acre"}];
    rows=[["Entered measurement",source,"Selected calculation mode."],["Exact acre relationship","1 acre = 43,560 sq ft","NIST area conversion."],["Acres",`${F(p.acres,10)} acres`,"Square feet divided by 43,560."],["Square feet",`${F(p.squareFeet,6)} sq ft`,"Base US area used for conversion."],["Square yards",`${F(p.squareYards,6)} sq yd`,"Square feet divided by 9."],["Square meters",`${F(p.squareMeters,6)} sq m`,"Square feet x 0.09290304."],["Hectares",`${F(p.hectares,10)} ha`,"Square meters divided by 10,000."],["Square miles",`${F(p.squareMiles,12)} sq mi`,"Acres divided by 640."],["Equivalent square side",`${F(p.sideFeet,4)} ft`,"Square root of area; shape illustration only."]];
  } else if (engine === "stones_pounds_advanced") {
    ...
    else{const date=value=>value.toLocaleDateString('en-US',{year:'numeric',month:'short',day:'numeric'}),gainLabel=p.gain>=0?'Net gain':'Net loss',depositCount=p.activities.filter(item=>item.kind==='deposit').length,withdrawalCount=p.activities.filter(item=>item.kind==='withdrawal').length;cards=[["Money-weighted return",`${p.annualized>=0?'+':''}${F(p.annualized,2)}%`,"Annualized using exact cash-flow dates."],[gainLabel,USD(Math.abs(p.gain)),"Ending value minus net invested cash."],["Account period",`${F(p.years,2)} years`,`${date(p.start)} to ${date(p.end)}.`],["Simple adjusted return",`${p.simpleReturn>=0?'+':''}${F(p.simpleReturn,2)}%`,"Not adjusted for cash-flow timing."]];bars=[{label:"Starting balance",value:p.starting,display:USD(p.starting)},{label:"Deposits",value:p.deposits,display:USD(p.deposits)},{label:"Withdrawals",value:p.withdrawals,display:USD(p.withdrawals)},{label:"Ending balance",value:p.ending,display:USD(p.ending)}];rows=[["Starting account",date(p.start),USD(p.starting)],["Deposits",`${F(depositCount,0)} ${depositCount===1?'activity':'activities'}`,USD(p.deposits)],["Withdrawals",`${F(withdrawalCount,0)} ${withdrawalCount===1?'activity':'activities'}`,USD(p.withdrawals)],["Ending account",date(p.end),USD(p.ending)],["Net invested cash",USD(p.netInvested),"Start + deposits - withdrawals"],[gainLabel,USD(Math.abs(p.gain)),"End - net invested cash"],["Money-weighted annual return",`${p.annualized>=0?'+':''}${F(p.annualized,4)}%`,"Solved from dated cash flows"]]}
  } else if (engine === "debt_payoff_advanced") {
  ...
  } else if (engine === "area_advanced") {
    const p=geometryAreaProjection();
    if(!p.valid){cards=[["Result","Check measurements",p.message],["Area","Unavailable","Positive dimensions are required."],["Perimeter","Unavailable","Correct the inputs first."],["Shape","Unavailable","Choose a supported shape."]];bars=[];rows=[["Validation","Unable to calculate",p.message]]}
    else{const perimeterLabel=p.shape==='circle'?"Circumference":p.shape==='ellipse'?"Approx. perimeter":p.shape==='sector'?"Sector perimeter":"Perimeter";cards=[["Calculated area",`${F(p.result,10)} ${p.outputUnit[1]}`,p.shapeLabel],["Area in input units",`${F(p.area,10)} ${p.inputUnit[1]}²`,"Before output-unit conversion."],[perimeterLabel,p.perimeter===null?"Needs more measurements":`${F(p.perimeter,8)} ${p.inputUnit[1]}`,p.perimeter===null?"Entered dimensions do not determine it.":"Linear boundary measurement."],["Formula",p.formula,"Applied before unit conversion."]];bars=p.measures.map(item=>({label:item[0],value:item[1],display:`${F(item[1],6)} ${p.inputUnit[1]}`}));rows=[["Shape",p.shapeLabel,"Selected geometry."],["Formula",p.formula,"Area equation."],["Input unit",p.inputUnit[0],"Applies to every length field."],["Area in input units",`${F(p.area,10)} ${p.inputUnit[1]}²`,"Formula result before conversion."],["Selected output",`${F(p.result,10)} ${p.outputUnit[1]}`,p.outputUnit[0]+"."],["Square meters",`${F(p.squareMeters,10)} m²`,"Shared conversion basis."],[perimeterLabel,p.perimeter===null?"Not determined":`${F(p.perimeter,10)} ${p.inputUnit[1]}`,p.shape==='ellipse'?"Ramanujan approximation.":p.perimeter===null?"Additional side lengths are required.":"Boundary length."]];p.equivalents.forEach(item=>rows.push([item.name,`${F(item.value,10)} ${item.symbol}`,item.id===p.outputKey?"Selected output unit.":"Equivalent area."]))}
  } else if (engine === "unit_converter_advanced") {
    const p=unitConversionProjection();
    if(!p.valid){cards=[["Result","Check the value",p.message],["Category","Unavailable","Choose valid units."],["Reverse conversion","Unavailable","Correct the input first."],["Base value","Unavailable","Correct the input first."]];bars=[];rows=[["Validation","Unable to convert",p.message]]}
    else{cards=[["Converted value",`${F(p.result,10)} ${p.toUnit[1]}`,`${F(p.value,10)} ${p.fromUnit[1]} entered.`],["Unit relationship",p.reverseText,"Reference relationship."],["Base-unit value",`${F(p.base,10)} ${p.config.units[p.config.base][1]}`,`Internal ${p.config.label.toLowerCase()} reference.`],["Category",p.config.label,`${p.equivalents.length} supported units.`]];bars=[];rows=p.equivalents.map(item=>[`${item.name} (${item.symbol})`,`${F(item.value,10)} ${item.symbol}`,item.id===p.from?"Entered unit":item.id===p.to?"Selected result unit":"Equivalent value"])}
  } else if (engine === "depreciation_advanced") {
    const p=depreciationProjection(),schedule=document.querySelector('#depreciationSchedule tbody');
    if(!p.valid){cards=[["Result","Check inputs",p.message],["Annual depreciation","Unavailable","Correct the inputs first."],["Book value","Unavailable","Correct the inputs first."],["Schedule","Unavailable","Correct the inputs first."]];bars=[];rows=[["Validation","Unable to calculate",p.message]];if(schedule)schedule.innerHTML=''}
    else{const sampleYears=[0,Math.ceil(p.life*.25),Math.ceil(p.life*.5),Math.ceil(p.life*.75),p.life],seen=new Set();bars=sampleYears.filter(year=>!seen.has(year)&&seen.add(year)).map(year=>({label:`End of year ${year}`,value:year===0?p.cost:p.schedule[year-1].ending,display:USD(year===0?p.cost:p.schedule[year-1].ending)}));cards=[[`Year ${p.selectedYear} depreciation`,USD(p.selected.expense),p.methodLabel+" method."],["Ending book value",USD(p.selected.ending),`After year ${p.selectedYear}.`],["Accumulated depreciation",USD(p.selected.accumulated),"Cost allocated through selected year."],["Depreciable basis",USD(p.basis),"Cost minus salvage value."]];rows=[["Method",p.methodLabel,p.method==='declining'?`${F(p.rate*100,4)}% entered annual rate.`:p.method==='double'?`${F(p.rate*100,4)}% rate from 2 / useful life.`:p.method==='syd'?"Accelerated remaining-life fraction.":"Equal full-year expense."],["Asset cost",USD(p.cost),"Entered book cost."],["Salvage value",USD(p.salvage),"Minimum book value in this model."],["Useful life",`${p.life} years`,"Full-year schedule."],["Selected year",F(p.selectedYear,0),"Year summarized above."],["Opening book value",USD(p.selected.opening),`Start of year ${p.selectedYear}.`],["Annual depreciation",USD(p.selected.expense),"Expense for selected year."],["Accumulated depreciation",USD(p.selected.accumulated),"Through selected year."],["Ending book value",USD(p.selected.ending),`End of year ${p.selectedYear}.`],["End-of-life book value",USD(p.endingBook),p.endingBook>p.salvage+.005?"Custom rate leaves value above salvage.":"At the entered salvage value."]];if(schedule)schedule.innerHTML=p.schedule.map(x=>`<tr><td>${x.year}</td><td>${USD(x.opening)}</td><td>${USD(x.expense)}</td><td>${USD(x.accumulated)}</td><td>${USD(x.ending)}</td></tr>`).join('')}
  } else if (engine === "triangle_advanced") {
    const p=triangleProjection();
    if(!p.valid){cards=[["Result","Check measurements",p.message],["Area","Unavailable","A valid triangle is required."],["Perimeter","Unavailable","A valid triangle is required."],["Angles","Unavailable","A valid triangle is required."]];bars=[{label:"Valid triangle",value:0,display:"No"}];rows=[["Validation","Unable to calculate",p.message]]}
    else if(p.mode==='area'){cards=[["Area",`${F(p.area,8)} square units`,"Base x perpendicular height / 2."],["Base",F(p.base,6),"Entered base length."],["Height",F(p.height,6),"Perpendicular to the base."],["Other measures","Need more data","Sides and angles are not unique."]];bars=[{label:"Base",value:p.base,display:F(p.base,4)},{label:"Height",value:p.height,display:F(p.height,4)}];rows=[["Calculation mode","Base and height","Area-only mode."],["Base",F(p.base,8),"Entered length."],["Perpendicular height",F(p.height,8),"Entered height."],["Area",F(p.area,8),"Base x height / 2."],["Perimeter and angles","Not determined","Base and height alone do not define a unique triangle."]]}
    else{const modeLabel=p.mode==='sss'?"Three sides (SSS)":p.mode==='sas'?"Two sides and included angle (SAS)":"Right triangle";cards=[["Area",`${F(p.area,8)} square units`,p.mode==='sss'?"Heron's formula.":p.mode==='sas'?"ab sin(C) / 2.":"ab / 2."],["Perimeter",F(p.perimeter,8),"a + b + c."],["Largest side",F(Math.max(p.a,p.b,p.c),8),"Longest of the solved sides."],["Largest angle",`${F(Math.max(p.A,p.B,p.C),6)}°`,"Opposite the largest side."]];bars=[{label:"Side a",value:p.a,display:F(p.a,6)},{label:"Side b",value:p.b,display:F(p.b,6)},{label:"Side c",value:p.c,display:F(p.c,6)}];rows=[["Calculation mode",modeLabel,"Selected known measurements."],["Side a",F(p.a,8),`Opposite angle A (${F(p.A,6)}°).`],["Side b",F(p.b,8),`Opposite angle B (${F(p.B,6)}°).`],["Side c",F(p.c,8),`Opposite angle C (${F(p.C,6)}°).`],["Area",F(p.area,8),"Square units."],["Perimeter",F(p.perimeter,8),"Sum of all three sides."],["Semiperimeter",F(p.semiperimeter,8),"Perimeter divided by 2."],["Altitude to side a",F(p.altitudeA,8),"2 x area / a."],["Altitude to side b",F(p.altitudeB,8),"2 x area / b."],["Altitude to side c",F(p.altitudeC,8),"2 x area / c."],["Inradius",F(p.inradius,8),"Area / semiperimeter."],["Circumradius",F(p.circumradius,8),"abc / (4 x area)."]]}
